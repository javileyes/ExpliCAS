// New inverse trig identity rules to add to trigonometry.rs
...
define_rule!(InverseTrigAtanRule, "Inverse Tan Relations", |ctx, expr| {
    if let Expr::Add(l, r) = ctx.get(expr) {
        let l_data = ctx.get(*l).clone();
        let r_data = ctx.get(*r).clone();

        // arctan(x) + arctan(1/x) = π/2
        if let (Expr::Function(l_name, l_args), Expr::Function(r_name, r_args)) = (l_data, r_data) {
            if l_name == "arctan" && r_name == "arctan" && l_args.len() == 1 && r_args.len() == 1 {
                let x = l_args[0];
