#!/usr/bin/env bash
# Lint: Eval Wire Canonical Enforcement
#
# This script ensures that CLI and FFI route eval wire through session-layer
# canonical entry points and do not call solver wire helpers directly.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

echo "=== Eval Wire Canonical Enforcement ==="
...
elif grep -rq "evaluate_eval_canonical" "$ROOT/crates/cas_cli/src"; then
...
if grep -rq "evaluate_eval_canonical" "$ROOT/crates/cas_android_ffi/src"; then
    echo "✔ FFI routes eval-json via cas_session canonical bridge"
else
    echo "✘ ERROR: FFI must call evaluate_eval_canonical"
    ERRORS=$((ERRORS + 1))
fi
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Eval wire must use canonical cas_session entry points"
    exit 1
fi

echo ""
echo "✔ eval wire canonical enforcement passed"
