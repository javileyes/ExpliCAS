#!/bin/bash
# Rewrite Migration Script V9 - Handles crate::rule::Rewrite patterns
...
process_file() {
    ...
    perl -0777 -pe '
        # Pattern 1: crate::rule::Rewrite with string desc, shorthand new_expr
        ...
        \}\s*\)/return Some(crate::rule::Rewrite::new(new_expr).desc("$1"))/gsx;
        
        # Pattern 2: crate::rule::Rewrite with string desc, explicit new_expr: var
        s/return\s+Some\s*\(\s*crate::rule::Rewrite\s*\{\s*
            new_expr\s*:\s*(\w+)\s*,\s*
            description\s*:\s*"([^"]+)"\.to_string\(\)\s*,\s*
            before_local\s*:\s*None\s*,\s*
            ...
            required_conditions\s*:\s*vec!\[\s*\]\s*,\s*
            poly_proof\s*:\s*None\s*,?\s*
        \}\s*\)/return Some(crate::rule::Rewrite::new($1).desc("$2"))/gsx;
        
        # Pattern 3: Some(crate::rule::Rewrite without return, shorthand
        s/Some\s*\(\s*crate::rule::Rewrite\s*\{\s*
            ...
            required_conditions\s*:\s*vec!\[\s*\]\s*,\s*
            poly_proof\s*:\s*None\s*,?\s*
        \}\s*\)/Some(crate::rule::Rewrite::new(new_expr).desc("$1"))/gsx;
        
        # Pattern 4: Some(crate::rule::Rewrite without return, explicit
        s/Some\s*\(\s*crate::rule::Rewrite\s*\{\s*
            new_expr\s*:\s*(\w+)\s*,\s*
            description\s*:\s*"([^"]+)"\.to_string\(\)\s*,\s*
            before_local\s*:\s*None\s*,\s*
            ...
            required_conditions\s*:\s*vec!\[\s*\]\s*,\s*
            poly_proof\s*:\s*None\s*,?\s*
        \}\s*\)/Some(crate::rule::Rewrite::new($1).desc("$2"))/gsx;
