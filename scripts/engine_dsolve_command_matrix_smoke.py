...
DEFAULT_DSOLVE_COMMAND_MATRIX_CASES = (
    ...
    DsolveCommandMatrixCase(
        name="residual_nonexact_nonlinear",
        expr="dsolve((y + x*y^2) + x*diff(y,x) = 0, y, x)",
        expected_result="dsolve(x·diff(y, x) + x·y^2 + y = 0, y, x)",
        expected_warning_substrings=("no es separable, lineal ni exacta",),
        family="exacta",
        verification_regime="declined",
        outcome="residual",
        residual_cause="nonconservative_field",
        trace_regime="none",
    ...
    DsolveCommandMatrixCase(
        name="residual_ivp_derivative_condition_future_cycle",
        expr="dsolve(diff(y,x)=-y, y, x, y'(0)=3)",
        expected_result="dsolve(diff(y, x) = -y, y, x)",
        expected_warning_substrings=("ciclo O4",),
        verification_regime="declined",
        constant_regime="ivp_declined",
        outcome="residual",
        residual_cause="derivative_condition_future_cycle",
        trace_regime="none",
        presentation_regime="echo",
    ),
    DsolveCommandMatrixCase(
