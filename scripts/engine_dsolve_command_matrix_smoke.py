...
DEFAULT_DSOLVE_COMMAND_MATRIX_CASES = (
    ...
    DsolveCommandMatrixCase(
        name="system_2x2_real_eigenvalues",
        expr="dsolve([diff(x,t)=y, diff(y,t)=x], [x,y], t)",
        expected_result="{ x = C1·e^t + C2 / e^t, y = C1·e^t - C2 / e^t }",
        expected_warning_substrings=("constantes arbitrarias",),
        expected_solve_step_substrings=("sistema lineal X'", "Característica del sistema"),
        family="sistema",
        verification_regime="verified_per_basis",
        trace_regime="eigen_narrated",
        presentation_regime="component_pair",
    ),
    DsolveCommandMatrixCase(
        name="system_2x2_complex_real_solutions",
        expr="dsolve([diff(x,t)=-y, diff(y,t)=x], [x,y], t)",
        expected_result="{ x = -C1·cos(t) - C2·sin(t), y = C2·cos(t) - C1·sin(t) }",
        family="sistema",
        verification_regime="verified_per_basis",
        trace_regime="eigen_narrated",
        presentation_regime="component_pair",
    ...
    DsolveCommandMatrixCase(
        name="system_2x2_defective_generalized_vector",
        expr="dsolve([diff(x,t)=2*x+y, diff(y,t)=2*y], [x,y], t)",
        expected_result="{ x = C1·e^(2·t) + C2·t·e^(2·t), y = C2·e^(2·t) }",
        family="sistema",
        verification_regime="verified_per_basis",
        trace_regime="eigen_narrated",
        presentation_regime="component_pair",
    ...
    DsolveCommandMatrixCase(
        name="residual_system_nonlinear",
        expr="dsolve([diff(x,t)=x+y, diff(y,t)=x*y], [x,y], t)",
        expected_result="dsolve(diff(x, t) = x + y, diff(y, t) = x·y, t)",
        expected_warning_substrings=("no es lineal homogéneo",),
        family="sistema",
        verification_regime="declined",
        outcome="residual",
        residual_cause="system_not_linear_constant",
        trace_regime="none",
        presentation_regime="echo",
    ),
    DsolveCommandMatrixCase(
        name="residual_system_ivp_future_cycle",
        expr="dsolve([diff(x,t)=y, diff(y,t)=x], [x,y], t, x(0)=1)",
        expected_result="dsolve(diff(x, t) = y, diff(y, t) = x, t)",
        expected_warning_substrings=("ciclo futuro",),
        family="sistema",
        verification_regime="declined",
        constant_regime="ivp_declined",
        outcome="residual",
        residual_cause="system_conditions_future_cycle",
        trace_regime="none",
        presentation_regime="echo",
    ),
    DsolveCommandMatrixCase(
        name="cauchy_euler_distinct_roots",
        expr="dsolve(x^2*diff(y,x,2)+x*diff(y,x)-y=0, y, x)",
        expected_result="y = C2 / x + C1·x",
        expected_warning_substrings=("dominio x > 0",),
        expected_solve_step_substrings=("Cauchy-Euler", "ecuación indicial"),
        family="cauchy_euler",
        order_regime="second",
        verification_regime="verified_per_basis",
        trace_regime="indicial_narrated",
    ),
    DsolveCommandMatrixCase(
        name="cauchy_euler_double_root_ln",
        expr="dsolve(x^2*diff(y,x,2)-x*diff(y,x)+y=0, y, x)",
        expected_result="y = x·(C2·ln(x) + C1)",
        family="cauchy_euler",
        order_regime="second",
        verification_regime="verified_per_basis",
        trace_regime="indicial_narrated",
    ...
    DsolveCommandMatrixCase(
        name="cauchy_euler_complex_trig_ln",
        expr="dsolve(x^2*diff(y,x,2)+x*diff(y,x)+y=0, y, x)",
        expected_result="y = C1·cos(ln(x)) + C2·sin(ln(x))",
        family="cauchy_euler",
        order_regime="second",
        verification_regime="verified_per_basis",
        trace_regime="indicial_narrated",
    ...
    DsolveCommandMatrixCase(
        name="integrating_factor_mu_x_boyce",
        expr="dsolve((3*x*y+y^2) + (x^2+x*y)*diff(y,x) = 0, y, x)",
        expected_result="2·y·x^3 + x^2·y^2 = C",
        expected_warning_substrings=("μ(x) = x", "soluciones singulares"),
        expected_solve_step_substrings=(
            "factor integrante simple μ(x) = x",
            "Identificar forma exacta",
        ),
        family="factor_integrante",
        verification_regime="verified_per_component",
        trace_regime="mu_then_potential_narrated",
        presentation_regime="implicit",
    ),
    DsolveCommandMatrixCase(
        name="integrating_factor_mu_y_by_parts",
        expr="dsolve(y + (2*x-y*exp(y))*diff(y,x) = 0, y, x)",
        expected_result="2·y·e^y + e^y·(-y^2 - 2) + x·y^2 = C",
        expected_warning_substrings=("μ(y) = y",),
        family="factor_integrante",
        verification_regime="verified_per_component",
        trace_regime="mu_then_potential_narrated",
        presentation_regime="implicit",
    ...
    DsolveCommandMatrixCase(
        name="residual_mu_rational_potential_ceiling",
        expr="dsolve(2*x*y + (y^2-x^2)*diff(y,x) = 0, y, x)",
        expected_result=(
            "dsolve(diff(y, x)·(y^2 - x^2) + 2·x·y = 0, y, x)"
        ),
        expected_warning_substrings=("no casa ningún método clásico",),
        family="factor_integrante",
        outcome="residual",
        residual_cause="mu_rational_potential_out_of_reach",
        verification_regime="declined",
        trace_regime="residual_echo",
        presentation_regime="echo",
    ),
    DsolveCommandMatrixCase(
