#!/bin/bash
# Rewrite Migration Script V11 - Handles local() + format!() with Default::default()
# Also handles patterns with explicit new_expr: var and local()
...
process_file() {
    ...
    perl -0777 -pe '
        # Pattern 1: return Some(Rewrite { with local() + format!(), Default::default(), explicit new_expr
        ...
        # Pattern 2: return Some(Rewrite { with local() + format!(), Default::default(), shorthand new_expr
        s/return\s+Some\s*\(\s*Rewrite\s*\{\s*
            new_expr\s*,\s*
            description\s*:\s*(format!\([^)]+\))\s*,\s*
            before_local\s*:\s*Some\(\s*(\w+)\s*\)\s*,\s*
        ...
        # Pattern 3: Some(Rewrite { local() + format!() (without return), explicit
        ...
        # Pattern 4: Some(Rewrite { local() + format!() (without return), shorthand
        s/Some\s*\(\s*Rewrite\s*\{\s*
            new_expr\s*,\s*
            description\s*:\s*(format!\([^)]+\))\s*,\s*
            before_local\s*:\s*Some\(\s*(\w+)\s*\)\s*,\s*
