use cas_solver_core::rationalize_policy::{AutoRationalizeLevel, RationalizeOutcome};
...
pub(super) fn push_rationalize_stats(
    ...
    lines.push(format!(
        "  Rationalize: {:?}",
        stats.rationalize_level.unwrap_or(AutoRationalizeLevel::Off)
    ));

    if let Some(ref outcome) = stats.rationalize_outcome {
        match outcome {
            RationalizeOutcome::Applied => {
                lines.push("              → Applied ✓".to_string());
            }
            RationalizeOutcome::NotApplied(reason) => {
