//! Solution verification for solver outputs.
//!
//! This mirrors `cas_engine::solver::check` while living in the solver facade
//! crate to reduce coupling to engine-internal modules.

use cas_ast::Context;
use cas_ast::{Equation, ExprId, SolutionSet};
use cas_engine::{DomainMode, EvalConfig, PhaseBudgets, SharedSemanticConfig, Simplifier};
...
/// Verify a full solution set against the original equation.
...
fn simplify_options_for_domain(domain_mode: DomainMode) -> cas_engine::SimplifyOptions {
    cas_engine::SimplifyOptions {
...
fn fold_numeric_islands(ctx: &mut Context, root: ExprId) -> ExprId {
    let fold_opts = cas_engine::SimplifyOptions {
        collect_steps: false,
        expand_mode: false,
        shared: SharedSemanticConfig {
            semantics: EvalConfig {
