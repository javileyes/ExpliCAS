...
pub fn solve_with_display_steps(
    ...
    let ctx = crate::solve::SolveCtx::default();
...
pub(super) fn solve_inner(
    ...
    simplifier: &mut Simplifier,
    opts: SolverOptions,
    parent_ctx: &crate::solve::SolveCtx,
) -> Result<(SolutionSet, Vec<SolveStep>), CasError> {
    ...
    execute_unwrap_strategy_with_default_route_and_residual_hint_and_unified_step_mapper_with_state(
        ...
        |core_ctx, base, other_side| {
            cas_solver_core::log_domain::classify_log_solve_with_env_and_tri_prover(
                ...
                &ctx.domain_env,
                |inner_ctx, expr| {
                    crate::solve::prove_positive_core(inner_ctx, expr, opts.value_domain)
                },
            )
        },
        context_render_expr,
