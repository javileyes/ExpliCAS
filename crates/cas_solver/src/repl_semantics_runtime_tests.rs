...
mod tests {
    use crate::{
        ...
        evaluate_context_command_on_runtime, evaluate_context_command_with_config_sync_on_runtime,
        evaluate_semantics_command_on_runtime,
        evaluate_semantics_command_with_config_sync_on_runtime, EvalOptions, ExpandPolicy,
        ReplSemanticsRuntimeContext, Simplifier, SimplifyOptions,
    };
    ...
    fn apply_context_command_on_runtime_updates_mode() {
        ...
        let out = apply_context_command_on_runtime("context solve", &mut runtime);
    ...
    fn apply_autoexpand_command_on_runtime_updates_policy() {
        let mut runtime = MockReplSemanticsRuntime::new();
        let out = apply_autoexpand_command_on_runtime("autoexpand on", &mut runtime);
        assert!(out.rebuilt_simplifier);
        assert_eq!(
    ...
    fn apply_semantics_command_on_runtime_updates_domain() {
        let mut runtime = MockReplSemanticsRuntime::new();
        let out = apply_semantics_command_on_runtime("semantics set domain assume", &mut runtime);
    ...
    fn evaluate_context_command_on_runtime_runs_rebuild_hook() {
        ...
        let message =
            evaluate_context_command_on_runtime("context solve", &mut runtime, |_runtime| {
                hook_called = true
            });
    ...
    fn evaluate_autoexpand_command_on_runtime_runs_rebuild_hook() {
        ...
        let mut hook_called = false;
        let message =
            evaluate_autoexpand_command_on_runtime("autoexpand on", &mut runtime, |_runtime| {
                hook_called = true
            });
    ...
    fn evaluate_semantics_command_on_runtime_runs_rebuild_hook() {
        let mut runtime = MockReplSemanticsRuntime::new();
        let mut hook_called = false;
        let message = evaluate_semantics_command_on_runtime(
