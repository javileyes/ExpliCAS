...
fn parse_solve_invocation_check_honors_flag() {
    let (check, tail) = crate::parse_solve_invocation_check("--check x+1=2, x", false);
    assert!(check);
...
fn parse_solve_command_input_accepts_comma_form() {
    let parsed = crate::parse_solve_command_input("x + 2 = 5, x");
    assert_eq!(
        parsed,
...
fn parse_timeline_command_input_routes_solve() {
    let parsed = crate::parse_timeline_command_input("solve x + 2 = 5, x");
    assert_eq!(
        parsed,
...
fn prepare_timeline_solve_equation_requires_equation() {
    ...
    let err = crate::prepare_timeline_solve_equation(&mut ctx, "x + 2", Some("x".to_string()))
