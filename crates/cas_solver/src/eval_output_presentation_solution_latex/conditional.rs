...
use cas_formatter::StylePreferences;

pub(super) fn render_conditional_solution_set(
    ctx: &Context,
    cases: &[Case],
    ...
    if filtered_cases.is_empty() {
        return cases
            .first()
            .map(|case| super::solution_set_to_output_latex(ctx, &case.then.solutions, style))
            .unwrap_or_else(|| r"\emptyset".to_string());
    }

    let mut latex_parts = Vec::new();
    for case in &filtered_cases {
        let inner_latex = super::solution_set_to_output_latex(ctx, &case.then.solutions, style);
    ...
    if filtered_cases.len() == 1 {
        ...
        if case.when.is_empty() {
            return super::solution_set_to_output_latex(ctx, &case.then.solutions, style);
        }
    }
...
mod tests {
    ...
    fn conditional_latex_skips_pure_residual_otherwise_but_keeps_guard() {
        ...
        let rendered = render_conditional_solution_set(
            &ctx,
            &[guarded, otherwise_residual],
