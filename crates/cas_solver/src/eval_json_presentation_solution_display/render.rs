...
use super::conditional::format_conditional_solution_set_eval_json;
use super::interval::format_interval_eval_json;
...
pub(crate) fn format_solution_set_eval_json(ctx: &Context, solution_set: &SolutionSet) -> String {
    match solution_set {
        ...
        SolutionSet::Conditional(cases) => format_conditional_solution_set_eval_json(ctx, cases),
        SolutionSet::Continuous(interval) => {
            format_interval_eval_json(ctx, interval.min, interval.max)
        }
        SolutionSet::Union(intervals) => {
            let parts: Vec<String> = intervals
                .iter()
                .map(|int| format_interval_eval_json(ctx, int.min, int.max))
                .collect();
            parts.join(" U ")
        }
