...
pub(super) fn format_conditional_solution_set_eval_json(
    ctx: &Context,
    cases: &[cas_ast::Case],
) -> String {
    ...
    for case in cases {
        ...
        let inner_str = super::render::format_solution_set_eval_json(ctx, &case.then.solutions);
    ...
    if parts.is_empty() && !cases.is_empty() {
        for case in cases {
            if !case.when.is_empty() {
                let inner_str =
                    super::render::format_solution_set_eval_json(ctx, &case.then.solutions);
                let cond_str = cas_formatter::condition_set_to_display(&case.when, ctx);
