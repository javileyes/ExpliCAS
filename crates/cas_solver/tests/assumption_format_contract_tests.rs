...
fn displayable_assumption_lines_for_step_hides_derived() {
    ...
    let mut step = cas_solver::Step::new("demo", "RuleA", x, x, vec![], Some(&ctx));
    step.meta_mut()
        .assumption_events
    ...
    let lines = cas_solver::format_displayable_assumption_lines_for_step(&step);
...
fn grouped_displayable_assumption_lines_keep_kind_order() {
    ...
    let mut step = cas_solver::Step::new("demo", "RuleB", x, x, vec![], Some(&ctx));
    step.meta_mut()
        .assumption_events
    ...
    let lines = cas_solver::format_displayable_assumption_lines_grouped_for_step(&step);
...
fn collect_assumed_conditions_from_steps_dedupes_by_key() {
    ...
    let mut step_a = cas_solver::Step::new("demo", "RuleA", x, x, vec![], Some(&ctx));
    step_a
        .meta_mut()
        .assumption_events
        .push(AssumptionEvent::positive(&ctx, x));

    let mut step_b = cas_solver::Step::new("demo", "RuleB", x, x, vec![], Some(&ctx));
    step_b
        .meta_mut()
        .assumption_events
        .push(AssumptionEvent::positive(&ctx, x));

    let items = cas_solver::collect_assumed_conditions_from_steps(&[step_a, step_b]);
