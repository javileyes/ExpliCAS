...
fn budget_exempt_allowlist() {
    ...
    let allowlist = [
        ...
        "integrate_rule.rs", // componentwise integrate over Matrix (COMPONENTWISE_MAX_CELLS=64; all-or-nothing)
    ];

    let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let rules_dir = [
        manifest_dir.join("src/rules"),
        manifest_dir.join("../cas_engine/src/rules"),
    ]
    .into_iter()
    .find(|p| p.exists())
    .expect("could not locate rules directory for allowlist scan");
    ...
    let engine_dir = [
        manifest_dir.join("src/engine"),
        manifest_dir.join("../cas_engine/src/engine"),
    ]
    .into_iter()
    .find(|p| p.exists())
    .expect("could not locate engine directory for allowlist scan");
