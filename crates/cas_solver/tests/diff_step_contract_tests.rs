...
fn arctan_positive_affine_sqrt_diff_cancels_external_post_calculus_coefficient() {
    ...
    assert_eq!(result, "1 / (2 * sqrt(2 * x + 3) * (x + 2))");
    assert!(
    ...
    assert!(
        !result.contains("2 * x + 4"),
        "post-calculus presentation should keep denominator content factored: {result}"
    );
        ...
        if input == "diff(arccot(sqrt(x)), x)" {
            assert!(
                output
                    .steps
                    .iter()
                    .any(|step| step.rule_name == "arccot(x) → arctan(1/x)"),
                "expected the arccot rewrite to remain visible"
            );
            assert!(
                output
                    .steps
...
fn inverse_hyperbolic_acosh_surd_polynomial_diff_uses_compact_real_domain() {
    ...
    let mut state = SessionState::new();
    state.options_mut().steps_mode = StepsMode::Off;
    let input = "diff(acosh((x^2+x)/sqrt(5)), x)";
    let parsed = parse(input, &mut engine.simplifier.context).expect("parse");
...
fn atanh_sqrt_diff_keeps_residual_when_real_open_interval_is_empty() {
    for input in [
        ...
        .collect();
        assert!(
            required.iter().all(|condition| !condition.contains("> 0")),
            "input: {input}, residual should not surface an impossible positive Requires: {required:?}"
        );
        assert!(
            output
                .steps
