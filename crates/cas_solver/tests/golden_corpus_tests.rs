...
fn load_corpus(filename: &str) -> Vec<String> {
    fn find_corpus_file(filename: &str) -> PathBuf {
        let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let local = manifest_dir.join("tests").join("corpus").join(filename);
        if local.exists() {
            return local;
        }
        manifest_dir
    ...
    let path = find_corpus_file(filename);
    let content = std::fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("Failed to load corpus {}: {}", path.display(), e));
    ...
    for cmd in &solve_commands {
        let result = catch_unwind(|| {
            ...
            if let Some((eq_str, var)) = rest.rsplit_once(',') {
                ...
                {
                    let _ = solve(&eq, var, &mut engine.simplifier);
                }
            }
        });
