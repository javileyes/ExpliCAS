...
fn infer_solve_variable_returns_single_free_variable() {
    ...
    let inferred = cas_solver::api::infer_solve_variable(&ctx, expr).expect("single variable");
...
fn infer_solve_variable_ignores_known_constants() {
    ...
    let inferred = cas_solver::api::infer_solve_variable(&ctx, expr).expect("no free variables");
...
fn infer_solve_variable_returns_sorted_ambiguous_variables() {
    ...
    let inferred =
        cas_solver::api::infer_solve_variable(&ctx, expr).expect_err("ambiguous variables");
