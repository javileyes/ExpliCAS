...
//!
//! This module builds display hints from simplification step snapshots.
//!
//! # Example
//! ```ignore
...
use crate::display_context::{DisplayContext, DisplayHint};
use cas_ast::{BuiltinFn, Context, Expr};

/// Minimal step view required to build display hints.
pub trait DisplayStepLike {
...
pub(crate) fn build_display_context(
    ctx: &Context,
    original_expr: cas_ast::ExprId,
    steps: &[impl DisplayStepLike],
) -> DisplayContext {
...
mod tests {
    ...
    struct TestStep {
        ...
        global_before: Option<cas_ast::ExprId>,
        global_after: Option<cas_ast::ExprId>,
    }

    impl DisplayStepLike for TestStep {
        fn rule_name(&self) -> &str {
            self.rule_name
        }
    ...
    fn test_extract_root_index_cbrt() {
        ...
        let x = ctx.var("x");
        let three = ctx.num(3);
        let cbrt_x = ctx.call_builtin(cas_ast::BuiltinFn::Sqrt, vec![x, three]);
