...
pub fn evaluate_autoexpand_command_input(
    ...
    match parse_autoexpand_command_input(line) {
        crate::autoexpand_command_types::AutoexpandCommandInput::ShowCurrent => {
            AutoexpandCommandResult::ShowCurrent {
                message: format_autoexpand_current_message(state.policy, state.budget),
            }
        }
        crate::autoexpand_command_types::AutoexpandCommandInput::SetPolicy(policy) => {
            AutoexpandCommandResult::SetPolicy {
                policy,
                message: format_autoexpand_set_message(policy, state.budget),
            }
        }
        crate::autoexpand_command_types::AutoexpandCommandInput::UnknownMode(mode) => {
            AutoexpandCommandResult::Invalid {
