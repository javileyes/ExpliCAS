...
mod tests {
    ...
    fn solver_toggle_round_trip_preserves_config_values() {
        let mut config = super::CasConfig {
            distribute: true,
        ...
        let toggles = super::solver_toggle_config_from_cas_config(&config);
        config = super::CasConfig::default();
        super::apply_solver_toggle_to_cas_config(&mut config, toggles);
