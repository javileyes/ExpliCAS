...
fn template_for(key: &str, lang: Language) -> Option<&'static str> {
    let (es, en) = match key {
        ...
        "limit.iterated" => ("Límite ITERADO: se evalúa el interior y después el exterior — no es el límite doble (pueden diferir)", "ITERATED limit: evaluate the inner limit first, then the outer — this is not the double limit (they can differ)"),
        "limit.squeeze" => ("Límite probado por acotación (squeeze): 0 ≤ |f| ≤ C·rᵖ con r² = x²+y², y rᵖ → 0", "Limit proven by squeezing: 0 ≤ |f| ≤ C·rᵖ with r² = x²+y², and rᵖ → 0"),
        "limit.path_counterexample" => ("El límite multivariable no existe: {0}", "The multivariate limit does not exist: {0}"),
        "potential.conservativity_check" => ("Comprobar que el campo es conservativo y reconstruir por caminos: φ = ∫F₁dx₁, luego integrar cada diferencia F_k − ∂φ/∂x_k", "Check the field is conservative and reconstruct along paths: φ = ∫F₁dx₁, then integrate each difference F_k − ∂φ/∂x_k"),
        "potential.reconstruct" => ("Verificación exacta: ∂φ/∂xᵢ ≡ Fᵢ en cada componente (la constante aditiva es implícita)", "Exact verification: ∂φ/∂xᵢ ≡ Fᵢ for every component (the additive constant is implicit)"),
