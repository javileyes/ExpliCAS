use cas_ast::{Context, Expr, ExprId, Constant};
...
impl<'a> AstVisualizer<'a> {
    ...
    fn write_node(&mut self, output: &mut String, id: ExprId) -> usize {
        ...
        match expr {
            Expr::Number(n) => {
                output.push_str(&format!("  n{} [label=\"{}\", shape=circle, fillcolor=\"#e3f2fd\"];\n", node_id, n));
            }
            Expr::Variable(v) => {
                output.push_str(&format!("  n{} [label=\"{}\", shape=ellipse, fillcolor=\"#c8e6c9\"];\n", node_id, v));
            }
            Expr::Constant(c) => {
                let label = match c {
                    Constant::Pi => "π",
                ...
                output.push_str(&format!("  n{} [label=\"{}\", shape=diamond, fillcolor=\"#fff9c4\"];\n", node_id, label));
            }
            Expr::Add(l, r) => {
                output.push_str(&format!("  n{} [label=\"+\", fillcolor=\"#fff3e0\"];\n", node_id));
            ...
            Expr::Mul(l, r) => {
                output.push_str(&format!("  n{} [label=\"×\", fillcolor=\"#ffe0b2\"];\n", node_id));
                let l_id = self.write_node(output, l);
                let r_id = self.write_node(output, r);
                output.push_str(&format!("  n{} -> n{} [label=\"L\"];\n", node_id, l_id));
                output.push_str(&format!("  n{} -> n{} [label=\"R\"];\n", node_id, r_id));
            ...
            Expr::Pow(b, e) => {
                output.push_str(&format!("  n{} [label=\"^\", fillcolor=\"#f8bbd0\"];\n", node_id));
            ...
            Expr::Neg(e) => {
                output.push_str(&format!("  n{} [label=\"−\", shape=triangle, fillcolor=\"#ffccbc\"];\n", node_id));
                let e_id = self.write_node(output, e);
                output.push_str(&format!("  n{} -> n{};\n", node_id, e_id));
            }
            Expr::Function(name, args) => {
                let escaped_name = name.replace("\"", "\\\"");
                output.push_str(&format!("  n{} [label=\"{}()\", shape=hexagon, fillcolor=\"#d1c4e9\"];\n", node_id, escaped_name));
                for (i, arg) in args.iter().enumerate() {
                    let arg_id = self.write_node(output, *arg);
                    output.push_str(&format!("  n{} -> n{} [label=\"arg{}\"];\n", node_id, arg_id, i));
                }
            }
        }
