...
pub fn isolate(lhs: ExprId, rhs: ExprId, op: RelOp, var: &str, simplifier: &mut Simplifier) -> Result<(SolutionSet, Vec<SolveStep>), CasError> {
    ...
    match lhs_expr {
        Expr::Variable(v) if v == var => {
            ...
            let set = match op {
                ...
                RelOp::Neq => {
                    // x != 5 -> (-inf, 5) U (5, inf)
                    let i1 = Interval { min: neg_inf(&mut simplifier.context), min_type: BoundType::Open, max: sim_rhs, max_type: BoundType::Open };
                    let i2 = Interval { min: sim_rhs, min_type: BoundType::Open, max: pos_inf(&mut simplifier.context), max_type: BoundType::Open };
                    SolutionSet::Union(vec![i1, i2])
                },
                RelOp::Lt => SolutionSet::Continuous(Interval {
                    min: neg_inf(&mut simplifier.context), min_type: BoundType::Open,
                    max: sim_rhs, max_type: BoundType::Open
                }),
                RelOp::Gt => SolutionSet::Continuous(Interval {
                    min: sim_rhs, min_type: BoundType::Open,
                    max: pos_inf(&mut simplifier.context), max_type: BoundType::Open
                }),
                RelOp::Leq => SolutionSet::Continuous(Interval {
                    min: neg_inf(&mut simplifier.context), min_type: BoundType::Open,
                    max: sim_rhs, max_type: BoundType::Closed
                }),
                RelOp::Geq => SolutionSet::Continuous(Interval {
                    min: sim_rhs, min_type: BoundType::Closed,
                    max: pos_inf(&mut simplifier.context), max_type: BoundType::Open
                }),
            };
            Ok((set, steps))
        ...
        Expr::Add(l, r) => {
            ...
            if contains_var(&simplifier.context, l, var) {
                ...
                let new_eq = Equation { lhs: l, rhs: new_rhs, op: op.clone() };
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Subtract {:?} from both sides", simplifier.context.get(r)),
                ...
                let (sim_rhs, _) = simplifier.simplify(new_rhs);
                let new_eq = Equation { lhs: r, rhs: new_rhs, op: op.clone() };
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Subtract {:?} from both sides", simplifier.context.get(l)),
                        equation_after: new_eq.clone(),
                    });
                }
        ...
        Expr::Sub(l, r) => {
            ...
            if contains_var(&simplifier.context, l, var) {
                ...
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Add {:?} to both sides", simplifier.context.get(r)),
                        equation_after: new_eq.clone(),
                    });
                }
                let results = isolate(l, sim_rhs, op, var, simplifier)?;
                ...
                let new_eq = Equation { lhs: r, rhs: new_rhs, op: new_op.clone() };
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Move {:?} and multiply by -1 (flips inequality)", simplifier.context.get(l)),
                        equation_after: new_eq.clone(),
                    });
                }
        ...
        Expr::Mul(l, r) => {
            ...
            if contains_var(&simplifier.context, l, var) {
                ...
                let new_eq = Equation { lhs: l, rhs: new_rhs, op: new_op.clone() };
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Divide both sides by {:?}", simplifier.context.get(r)),
                        equation_after: new_eq.clone(),
                    });
                }
                ...
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Divide both sides by {:?}", simplifier.context.get(l)),
                        equation_after: new_eq.clone(),
                    });
                }
        ...
        Expr::Div(l, r) => {
            // A / B = RHS
            if contains_var(&simplifier.context, l, var) {
                if contains_var(&simplifier.context, r, var) && matches!(op, RelOp::Lt | RelOp::Gt | RelOp::Leq | RelOp::Geq) {
                    ...
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Case 1: Assume {:?} > 0. Multiply by positive denominator.", simplifier.context.get(r)),
                            equation_after: Equation { lhs: l, rhs: sim_rhs, op: op_pos.clone() }
                        });
                    }
                    ...
                    let domain_eq = Equation { lhs: r, rhs: zero, op: RelOp::Gt };
                    ...
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Case 2: Assume {:?} < 0. Multiply by negative denominator (flips inequality).", simplifier.context.get(r)),
                    ...
                    // Domain: r < 0
                    let zero = simplifier.context.num(0);
                    let domain_eq_neg = Equation { lhs: r, rhs: zero, op: RelOp::Lt };
                    ...
                    all_steps.push(SolveStep { description: "--- End of Case 1 ---".to_string(), equation_after: Equation { lhs: l, rhs: sim_rhs, op: op.clone() }});
                    ...
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Multiply both sides by {:?}", simplifier.context.get(r)),
                            equation_after: new_eq.clone(),
                        });
                    }
                    let results = isolate(l, new_rhs, new_op, var, simplifier)?;
                ...
                if let Expr::Variable(v) = simplifier.context.get(r) {
                    if v == var && matches!(op, RelOp::Lt | RelOp::Gt | RelOp::Leq | RelOp::Geq) {
                         ...
                         if simplifier.collect_steps {
                             steps.push(SolveStep {
                                 description: format!("Case 1: Assume {:?} > 0. Multiply by {:?} (positive). Inequality direction preserved (flipped from isolation logic).", simplifier.context.get(r), simplifier.context.get(r)),
                         ...
                         let domain_pos = SolutionSet::Continuous(Interval {
                             min: simplifier.context.num(0), min_type: BoundType::Open,
                             max: pos_inf(&mut simplifier.context), max_type: BoundType::Open
                         });
                         ...
                         if simplifier.collect_steps {
                             steps.push(SolveStep {
                                 description: format!("Case 2: Assume {:?} < 0. Multiply by {:?} (negative). Inequality flips.", simplifier.context.get(r), simplifier.context.get(r)),
                         ...
                         let domain_neg = SolutionSet::Continuous(Interval {
                             min: neg_inf(&mut simplifier.context), min_type: BoundType::Open,
                             max: simplifier.context.num(0), max_type: BoundType::Open
                         });
                         ...
                         // Combine steps
                         let mut all_steps = steps_pos;
                         all_steps.push(SolveStep { description: "--- End of Case 1 ---".to_string(), equation_after: Equation { lhs: r, rhs: sim_rhs, op: op.clone() }}); // Dummy eq
                         all_steps.extend(steps_neg);
                ...
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Isolate denominator {:?}", simplifier.context.get(r)),
                        equation_after: new_eq.clone(),
                    });
                }
                let results = isolate(r, sim_rhs, op, var, simplifier)?;
        ...
        Expr::Pow(b, e) => {
            ...
            if contains_var(&simplifier.context, b, var) {
                ...
                if is_even {
                    ...
                    if is_negative(&simplifier.context, rhs) {
                        let result = match op {
                         ...
                         if simplifier.collect_steps {
                            steps.push(SolveStep {
                                description: format!("Even power cannot be negative ({:?} {} {:?})", simplifier.context.get(b), op, simplifier.context.get(rhs)),
                                equation_after: Equation { lhs, rhs, op: op.clone() }, // No change
                            });
                        }
                     ...
                     // Construct |B|
                     let abs_b = simplifier.context.add(Expr::Function("abs".to_string(), vec![b]));
                     
                     let new_eq = Equation { lhs: abs_b, rhs: new_rhs, op: op.clone() };
                     if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Take {:?}-th root of both sides (even root implies absolute value)", simplifier.context.get(e)),
                            equation_after: new_eq.clone(),
                        });
                     }
                    ...
                    let inv_exp = simplifier.context.add(Expr::Div(one, e));
                    let new_rhs = simplifier.context.add(Expr::Pow(rhs, inv_exp));
                    let new_eq = Equation { lhs: b, rhs: new_rhs, op: op.clone() };
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Take {:?}-th root of both sides", simplifier.context.get(e)),
                            equation_after: new_eq.clone(),
                        });
                    }
                ...
                // E = log(B, RHS)
                let new_rhs = simplifier.context.add(Expr::Function("log".to_string(), vec![b, rhs]));
                let new_eq = Equation { lhs: e, rhs: new_rhs, op: op.clone() };
                if simplifier.collect_steps {
                    steps.push(SolveStep {
                        description: format!("Take log base {:?} of both sides", simplifier.context.get(b)),
                        equation_after: new_eq.clone(),
                    });
                }
        ...
        Expr::Function(name, args) => {
            if name == "abs" && args.len() == 1 {
                ...
                let eq1 = Equation { lhs: arg, rhs: rhs, op: op.clone() };
                let mut steps1 = steps.clone();
                if simplifier.collect_steps {
                    steps1.push(SolveStep {
                        description: format!("Split absolute value (Case 1): {:?} {} {:?}", simplifier.context.get(arg), op, simplifier.context.get(rhs)),
                ...
                let eq2 = Equation { lhs: arg, rhs: neg_rhs, op: op2.clone() };
                let mut steps2 = steps.clone();
                if simplifier.collect_steps {
                    steps2.push(SolveStep {
                        description: format!("Split absolute value (Case 2): {:?} {} {:?}", simplifier.context.get(arg), op2, simplifier.context.get(neg_rhs)),
                ...
                let final_set = match op {
                    RelOp::Eq | RelOp::Neq | RelOp::Gt | RelOp::Geq => union_solution_sets(&simplifier.context, set1, set2),
                ...
                if contains_var(&simplifier.context, arg, var) && !contains_var(&simplifier.context, base, var) {
                    ...
                    let new_eq = Equation { lhs: arg, rhs: new_rhs, op: op.clone() };
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                            description: format!("Exponentiate both sides with base {:?}", simplifier.context.get(base)),
                            equation_after: new_eq.clone(),
                        });
                    }
                    let results = isolate(arg, new_rhs, op, var, simplifier)?;
                    prepend_steps(results, steps)
                } else if contains_var(&simplifier.context, base, var) && !contains_var(&simplifier.context, arg, var) {
                    let one = simplifier.context.num(1);
                    let inv_rhs = simplifier.context.add(Expr::Div(one, rhs));
                    let new_rhs = simplifier.context.add(Expr::Pow(arg, inv_rhs));
                    let new_eq = Equation { lhs: base, rhs: new_rhs, op: op.clone() };
                    if simplifier.collect_steps {
                        steps.push(SolveStep {
                ...
                if contains_var(&simplifier.context, arg, var) {
                    match name.as_str() {
                        ...
                        "exp" => {
                            let new_rhs = simplifier.context.add(Expr::Function("ln".to_string(), vec![rhs]));
                            let new_eq = Equation { lhs: arg, rhs: new_rhs, op: op.clone() };
                            if simplifier.collect_steps {
                                steps.push(SolveStep {
                        ...
                        "sin" => {
                            // sin(x) = y -> x = arcsin(y)
                            let new_rhs = simplifier.context.add(Expr::Function("arcsin".to_string(), vec![rhs]));
                            let new_eq = Equation { lhs: arg, rhs: new_rhs, op: op.clone() };
                            if simplifier.collect_steps {
                                steps.push(SolveStep {
                        ...
                        "cos" => {
                            // cos(x) = y -> x = arccos(y)
                            let new_rhs = simplifier.context.add(Expr::Function("arccos".to_string(), vec![rhs]));
                            let new_eq = Equation { lhs: arg, rhs: new_rhs, op: op.clone() };
                            if simplifier.collect_steps {
                                steps.push(SolveStep {
                        ...
                        "tan" => {
                            // tan(x) = y -> x = arctan(y)
                            let new_rhs = simplifier.context.add(Expr::Function("arctan".to_string(), vec![rhs]));
                            let new_eq = Equation { lhs: arg, rhs: new_rhs, op: op.clone() };
                            if simplifier.collect_steps {
                                steps.push(SolveStep {
...
pub fn simplify_rhs(rhs: ExprId, lhs: ExprId, op: RelOp, simplifier: &mut Simplifier) -> (ExprId, Vec<SolveStep>) {
