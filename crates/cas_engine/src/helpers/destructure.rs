...
#[inline]
#[allow(dead_code)]
pub(crate) fn fn_name_args(ctx: &Context, id: ExprId) -> Option<(&str, &[ExprId])> {
    match ctx.get(id) {
...
#[inline]
#[allow(dead_code)]
pub(crate) fn as_fn2(ctx: &Context, id: ExprId, name: &str) -> Option<(ExprId, ExprId)> {
    match ctx.get(id) {
