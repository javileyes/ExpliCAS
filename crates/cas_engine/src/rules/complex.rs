        ...
        if let Expr::Mul(l, r) = ctx.get(expr) {
            ...
            return Some(
                Rewrite::new(new_expr)
                    .desc("Gaussian multiplication: (a+bi)(c+di) = (ac-bd) + (ad+bc)i"),
            );
        }
        None
    }
...
define_rule!(
    ...
    |ctx, expr, parent_ctx| {
        ...
        if let Expr::Add(l, r) = ctx.get(expr) {
            ...
            return Some(
                Rewrite::new(new_expr).desc("Gaussian addition: (a+bi) + (c+di) = (a+c) + (b+d)i"),
            );
        }
        None
    }
...
define_rule!(
    ...
    |ctx, expr, parent_ctx| {
        ...
        if let Expr::Div(num, den) = ctx.get(expr) {
            ...
            return Some(
                Rewrite::new(new_expr).desc("Gaussian division: (a+bi)/(c+di) using conjugate"),
            );
        }
        None
    }
...
define_rule!(
    ...
    |ctx, expr, parent_ctx| {
        ...
        if let Expr::Function(fn_id, args) = ctx.get(expr) {
            if ctx.builtin_of(*fn_id) != Some(cas_ast::BuiltinFn::Sqrt) || args.len() != 1 {
                return None;
            }
