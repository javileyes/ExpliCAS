...
impl crate::rule::Rule for TrigSumToProductContractionRule {
    ...
    fn apply(
        ...
        let result = match (l_name.as_str(), is_add) {
            ("sin", true) => {
                // sin(a) + sin(b) → 2*sin((a+b)/2)*cos((a-b)/2)
                let sin_half_sum = ctx.add(Expr::Function("sin".to_string(), vec![half_sum_arg]));
                let cos_half_diff = ctx.add(Expr::Function("cos".to_string(), vec![half_diff_arg]));
                let product = ctx.add(Expr::Mul(sin_half_sum, cos_half_diff));
                ctx.add(Expr::Mul(two_id, product))
            }
            ("sin", false) => {
                // sin(a) - sin(b) → 2*cos((a+b)/2)*sin((a-b)/2)
                let cos_half_sum = ctx.add(Expr::Function("cos".to_string(), vec![half_sum_arg]));
                let sin_half_diff = ctx.add(Expr::Function("sin".to_string(), vec![half_diff_arg]));
