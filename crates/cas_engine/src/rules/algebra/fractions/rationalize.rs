        ...
        let rewrite =
            cas_math::div_expand_cancel_support::try_rewrite_div_expand_to_cancel_expr_with_thread_guards(
                ...
                |base_ctx, sub_frac| {
                    ...
                    simplifier.context = base_ctx.clone();
                    let (simplified, _) =
                    simplifier.simplify_with_options(sub_frac, super::strategy_simplify_options());
                    Some((simplifier.context, simplified))
                },
                crate::expand::expand,
                |expanded_ctx, expanded_num, expanded_den| {
                    ...
                    simplifier.context = expanded_ctx;
                    let (simplified_num, _) =
                        simplifier.simplify_with_options(expanded_num, super::strategy_simplify_options());
                    let (simplified_den, _) =
                        simplifier.simplify_with_options(expanded_den, super::strategy_simplify_options());
                    Some((simplifier.context, simplified_num, simplified_den))
                },
            )
