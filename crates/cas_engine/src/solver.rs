...
pub fn solve(eq: &Equation, var: &str, simplifier: &Simplifier) -> Result<(SolutionSet, Vec<SolveStep>), String> {
    ...
    if let Some(sub_var_expr) = detect_substitution(eq, var) {
        ...
        // We need a robust way to replace e^(2x) with u^2.
        // For now, let's just handle the specific case of e^(kx) -> u^k where u=e^x.
        let u_sym = "u";
...
fn detect_substitution(eq: &Equation, var: &str) -> Option<Rc<Expr>> {
...
fn collect_exponential_terms(expr: &Rc<Expr>, var: &str) -> Vec<Rc<Expr>> {
...
fn substitute_expr(expr: &Rc<Expr>, target: &Rc<Expr>, replacement_name: &str) -> Rc<Expr> {
    if expr == target {
