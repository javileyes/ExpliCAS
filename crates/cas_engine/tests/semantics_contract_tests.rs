...
fn simplify_options_includes_new_axes() {
    ...
    assert_eq!(opts.inv_trig, InverseTrigPolicy::Strict);
    assert_eq!(opts.value_domain, ValueDomain::RealOnly);
    assert_eq!(opts.branch, BranchPolicy::Principal);
}
...
fn eval_options_propagates_new_axes_to_simplify_options() {
    ...
    assert_eq!(simplify_opts.inv_trig, InverseTrigPolicy::Strict);
    assert_eq!(simplify_opts.value_domain, ValueDomain::RealOnly);
    assert_eq!(simplify_opts.branch, BranchPolicy::Principal);
}
