...
fn test_timeline_renders_all_medium_importance_steps() {
    ...
    let expected_count = display_steps.iter()
        .filter(|s| s.get_importance() >= ImportanceLevel::Medium)
        .count();
    
    eprintln!("Expected {} steps to be shown in Normal verbosity", expected_count);
    ...
    // Count Identity Property steps in HTML
    let identity_count = html.matches("Identity Property").count();
    eprintln!("Timeline HTML mentions 'Identity Property' {} times", identity_count);
    
    assert_eq!(step_card_count, expected_count, 
        "Timeline should render {} steps, but rendered {}", expected_count, step_card_count);
}
