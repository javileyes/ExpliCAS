...
#[test]
fn test_eval_json_diff_positive_quadratic_log_abs_pole_wrapper_without_depth_overflow() {
    let expr = "diff((ln(abs(x^2+x+1))+1/(x+2))/(x+2), x)";
    let (json, stderr) = eval_json_with_stderr(expr);
    ...
    assert!(
        !stderr.contains("depth_overflow"),
        "wrapper derivative should not emit depth_overflow for {expr}\nstderr:\n{stderr}"
    );
    ...
    assert!(
        required.iter().any(|condition| {
            condition["kind"] == "NonZero" && condition["expr_canonical"] == "x + 2"
        }),
        "diff shortcut should preserve the wrapper denominator guard for {expr}: {required:?}"
    );
}
