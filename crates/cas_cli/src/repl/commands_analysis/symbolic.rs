...
impl Repl {
    pub(crate) fn handle_equiv_core(&mut self, line: &str) -> ReplReply {
        match cas_session::solver_exports::evaluate_equiv_invocation_message_on_repl_core(
            &mut self.core,
            line,
        ) {
    ...
    pub(crate) fn handle_subst_core(&mut self, line: &str, verbosity: Verbosity) -> ReplReply {
        let message = match cas_session::solver_exports::evaluate_substitute_invocation_user_message_on_repl_core(
            &mut self.core,
            line,
    ...
    pub(crate) fn handle_explain_core(&mut self, line: &str) -> ReplReply {
        let message =
            match cas_session::solver_exports::evaluate_explain_invocation_message_on_repl_core(
                &mut self.core,
                line,
            ) {
                Ok(message) => message,
