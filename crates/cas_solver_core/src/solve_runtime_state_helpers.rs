//! Shared context/state helpers for solve runtime adapters.

use cas_ast::{symbol::SymbolId, Context, ExprId};

pub(crate) fn contains_var_in_context(ctx: &Context, expr: ExprId, var: &str) -> bool {
    crate::isolation_utils::contains_var(ctx, expr, var)
}

pub(crate) fn render_expr_in_context(ctx: &Context, expr: ExprId) -> String {
    cas_formatter::render_expr(ctx, expr)
}

pub(crate) fn zero_expr_in_context(ctx: &mut Context) -> ExprId {
    ctx.num(0)
}

pub(crate) fn is_known_negative_in_context(ctx: &Context, expr: ExprId) -> bool {
    crate::isolation_utils::is_known_negative(ctx, expr)
}

pub(crate) fn sym_name_as_string(ctx: &Context, fn_symbol: SymbolId) -> String {
    ctx.sym_name(fn_symbol).to_string()
}

pub(crate) fn simplify_rhs_with_step_pairs(
    simplified_rhs: ExprId,
    sim_steps: Vec<crate::step_model::Step>,
) -> (ExprId, Vec<(String, ExprId)>) {
    let entries = sim_steps
        .into_iter()
        .map(|step| (step.description.to_string(), step.after))
        .collect::<Vec<_>>();
    (simplified_rhs, entries)
}
