...
fn rational_over_expr(ctx: &mut Context, numerator: BigRational, denominator: ExprId) -> ExprId {
...
fn signed_sqrt_like_radicand(ctx: &Context, expr: ExprId) -> Option<(ExprId, BigRational)> {
    match ctx.get(expr) {
        Expr::Neg(inner) => {
...
fn sqrt_chain_argument_derivative_parts(
    ctx: &Context,
    arg: ExprId,
    var: &str,
...
fn arctan_sqrt_var_unit_shift_square_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn inverse_hyperbolic_scale_over_constant_root(
...
fn normalize_asinh_reciprocal_display_base(
    base: Polynomial,
    constant: BigRational,
...
fn positive_constant_difference(left: &Polynomial, right: &Polynomial) -> Option<BigRational> {
...
fn single_affine_factor(ctx: &Context, factors: &[ExprId], var: &str) -> Option<Polynomial> {
...
fn atanh_gap_constant_and_alignment(
    base: &Polynomial,
...
fn inverse_hyperbolic_sqrt_reciprocal_parts(
    ctx: &Context,
    expr: ExprId,
    var: &str,
...
fn inverse_hyperbolic_sqrt_reciprocal_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
    kind: InverseHyperbolicSqrtReciprocalKind,
...
#[derive(Clone, Copy)]
struct ReciprocalTrigDerivativePolicy {
...
fn reciprocal_trig_derivative_policy(
    denominator_builtin: BuiltinFn,
...
fn reciprocal_trig_derivative_policy_from_reciprocal(
    reciprocal_builtin: BuiltinFn,
) -> Option<ReciprocalTrigDerivativePolicy> {
...
fn build_reciprocal_trig_derivative_integral(
    ctx: &mut Context,
...
fn reciprocal_trig_derivative_base_antiderivative(
    ctx: &mut Context,
    denominator_builtin: BuiltinFn,
    arg: ExprId,
) -> Option<ExprId> {
...
fn trig_square_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn trig_tan_sixth_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn trig_tan_eighth_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn trig_cot_sixth_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn trig_cot_eighth_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn hyperbolic_tanh_even_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
fn hyperbolic_square_product_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn trig_tan_sixth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
    var: &str,
...
fn trig_tan_eighth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
    var: &str,
...
fn trig_cot_sixth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
    var: &str,
...
fn trig_cot_eighth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
    var: &str,
...
fn trig_sec_sixth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
) -> ExprId {
...
fn trig_sec_eighth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
) -> ExprId {
...
fn trig_csc_sixth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
) -> ExprId {
...
fn trig_csc_eighth_antiderivative_from_parts(
    ctx: &mut Context,
    arg: ExprId,
    a: BigRational,
) -> ExprId {
...
fn hyperbolic_odd_power_limited_affine_antiderivative(
    ctx: &mut Context,
    base: ExprId,
    exp: ExprId,
    var: &str,
...
#[derive(Clone, Copy)]
struct HyperbolicReciprocalDerivativePolicy {
    denominator_builtin: BuiltinFn,
    numerator_builtin: BuiltinFn,
}

#[derive(Clone, Copy)]
struct HyperbolicReciprocalTablePolicy {
    denominator_builtin: BuiltinFn,
    power: i64,
}

fn hyperbolic_reciprocal_table_policy(
    denominator_builtin: BuiltinFn,
    power: i64,
) -> Option<HyperbolicReciprocalTablePolicy> {
    if !matches!(denominator_builtin, BuiltinFn::Cosh | BuiltinFn::Sinh) || !matches!(power, 2 | 4)
    {
        return None;
    }
...
fn hyperbolic_reciprocal_derivative_policy(
    denominator_builtin: BuiltinFn,
...
fn build_hyperbolic_reciprocal_derivative_integral(
    ctx: &mut Context,
...
fn hyperbolic_reciprocal_power_antiderivative(
    ...
    let arg = reciprocal_hyperbolic_power_arg(ctx, den, policy.denominator_builtin, policy.power)?;
    ...
    match policy.power {
        2 => Some(build_hyperbolic_reciprocal_square_integral(
            ctx, policy, arg, scale,
        )),
        4 => Some(build_hyperbolic_reciprocal_fourth_integral(
            ctx, policy, arg, scale,
        )),
        _ => None,
    }
...
fn build_hyperbolic_reciprocal_square_integral(
    ...
    match policy.denominator_builtin {
        BuiltinFn::Cosh => {
            let tanh_arg = ctx.call_builtin(BuiltinFn::Tanh, vec![arg]);
            scale_expr_reciprocal_integration_result(ctx, scale, tanh_arg)
        }
        BuiltinFn::Sinh => {
            let cosh_arg = ctx.call_builtin(BuiltinFn::Cosh, vec![arg]);
            let sinh_arg = ctx.call_builtin(BuiltinFn::Sinh, vec![arg]);
            ...
            scale_expr_reciprocal_integration_result(ctx, negative_scale, quotient)
        }
...
fn build_hyperbolic_reciprocal_fourth_integral(
    ...
    match policy.denominator_builtin {
        BuiltinFn::Cosh => build_hyperbolic_cosh_reciprocal_fourth_integral(ctx, arg, scale),
        BuiltinFn::Sinh => build_hyperbolic_sinh_reciprocal_fourth_integral(ctx, arg, scale),
        _ => unreachable!("hyperbolic reciprocal table policy only supports sinh/cosh"),
    }
}
...
fn build_hyperbolic_cosh_reciprocal_fourth_integral(
    ...
    if let Expr::Number(scale_value) = ctx.get(scale).clone() {
        let linear_term = scale_rational_term(ctx, scale_value.clone(), tanh_arg);
    ...
    let linear_term = scale_expr_reciprocal_integration_result(ctx, scale, tanh_arg);
    let cubic_scale = scale_rational_term(ctx, BigRational::new(1.into(), 3.into()), scale);
    let cubic_term = scale_expr_reciprocal_integration_result(ctx, cubic_scale, tanh_cubed);
...
fn build_hyperbolic_sinh_reciprocal_fourth_integral(
    ...
    if let Expr::Number(scale_value) = ctx.get(scale).clone() {
        let linear_term = rational_over_expr(ctx, scale_value.clone(), tanh_arg);
        let cubic_term = rational_over_expr(
            ctx,
            -scale_value / BigRational::from_integer(3.into()),
            tanh_cubed,
        );
    ...
    let linear_term = scale_expr_reciprocal_integration_result(ctx, scale, coth_arg);
    let cubic_scale = scale_rational_term(ctx, BigRational::new(1.into(), 3.into()), scale);
    let cubic_term = scale_expr_reciprocal_integration_result(ctx, cubic_scale, coth_cubed);
    ctx.add(Expr::Sub(linear_term, cubic_term))
}
...
fn hyperbolic_reciprocal_derivative_antiderivative(
    ...
    let (den_builtin, arg) = reciprocal_hyperbolic_square_factor_parts(ctx, den)?;
    ...
    let factors = mul_leaves(ctx, num);
    let (numerator_index, _) = factors.iter().enumerate().find(|(_, factor)| {
        unary_builtin_arg(ctx, **factor, policy.numerator_builtin).is_some_and(|numerator_arg| {
            same_structural_or_linear_arg(ctx, numerator_arg, arg, var)
        })
    })?;
...
fn polynomial_reciprocal_trig_square_required_nonzero(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn scale_by_reciprocal_linear_coeff(ctx: &mut Context, integral: ExprId, coeff: ExprId) -> ExprId {
...
fn divide_by_coeff_unless_one_preserving_presentation(
    ctx: &mut Context,
    integral: ExprId,
    coeff: ExprId,
) -> ExprId {
...
fn add_linear_parts(ctx: &mut Context, left: ExprId, right: ExprId) -> ExprId {
...
fn sub_linear_parts(ctx: &mut Context, left: ExprId, right: ExprId) -> ExprId {
...
fn neg_linear_part(ctx: &mut Context, part: ExprId) -> ExprId {
...
fn negate_scalar_expr(ctx: &mut Context, expr: ExprId) -> ExprId {
    match ctx.get(expr).clone() {
...
fn polynomial_or_symbolic_linear_cofactor_scale(
    ctx: &mut Context,
    cofactor: ExprId,
    arg: ExprId,
    var: &str,
...
fn additive_var_dependent_part(ctx: &mut Context, expr: ExprId, var: &str) -> Option<ExprId> {
...
fn symbolic_linear_cofactor_scale_expr(
    ctx: &mut Context,
    cofactor: ExprId,
    arg: ExprId,
    var: &str,
...
fn same_structural_or_linear_arg(
    ctx: &mut Context,
    left: ExprId,
    right: ExprId,
    var: &str,
...
fn trig_reciprocal_derivative_cofactor_scale(
    ctx: &mut Context,
    cofactor: ExprId,
    arg: ExprId,
    var: &str,
...
fn trig_reciprocal_derivative_base_integral(
    ctx: &mut Context,
    den_builtin: BuiltinFn,
    arg: ExprId,
) -> Option<ExprId> {
...
fn polynomial_trig_reciprocal_derivative_parts(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn trig_reciprocal_derivative_cofactor_is_nonzero(
    ctx: &mut Context,
    cofactor: ExprId,
    arg: ExprId,
    var: &str,
...
fn sqrt_polynomial_derivative_quotient_scale_expr(
    ctx: &mut Context,
    numerator_factors: &[ExprId],
    denominator_factors: &[ExprId],
    sqrt_arg: ExprId,
...
fn polynomial_trig_reciprocal_factor_antiderivative(
    ...
    let factors = mul_leaves(ctx, num);
    let (numerator_index, _) = factors.iter().enumerate().find(|(_, factor)| {
        unary_builtin_arg(ctx, **factor, policy.derivative_builtin)
            .is_some_and(|numerator_arg| compare_expr(ctx, numerator_arg, arg) == Ordering::Equal)
    })?;
...
fn polynomial_sec_csc_square_required_nonzero(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
    ...
    Some(ctx.call_builtin(required_builtin, vec![arg]))
}

fn trig_ratio_square_required_nonzero(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn polynomial_antiderivative_expr(ctx: &mut Context, poly: &Polynomial) -> ExprId {
...
fn affine_args_are_opposite(ctx: &Context, left: ExprId, right: ExprId, var: &str) -> bool {
...
fn additive_quadratic_times_affine_ln_by_parts_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn is_positive_quadratic_polynomial(poly: &Polynomial) -> bool {
    if poly.degree() != 2 {
        return false;
    }
...
fn positive_quadratic_ln_by_parts_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn exp_trig_same_linear_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn polynomial_trig_linear_term_parts(
    ctx: &mut Context,
    term: ExprId,
    var: &str,
...
fn additive_polynomial_times_trig_linear_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn polynomial_hyperbolic_linear_term_parts(
    ctx: &mut Context,
    term: ExprId,
    var: &str,
) -> Option<(BuiltinFn, ExprId, ExprId, Polynomial)> {
...
fn additive_polynomial_times_hyperbolic_linear_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn reduce_surd_offset_by_square_denominator(
    arg_poly: &Polynomial,
    offset_square: &BigRational,
) -> Option<(Polynomial, BigRational, BigRational)> {
    if offset_square.is_integer() || offset_square <= &BigRational::zero() {
...
fn exact_polynomial_square_root(poly: &Polynomial) -> Option<Polynomial> {
    if poly.is_zero() {
        return None;
    }
...
fn compact_polynomial_square_arg(ctx: &mut Context, poly: &Polynomial) -> Option<ExprId> {
...
fn exact_positive_constant_minus_scaled_polynomial_square(
    poly: &Polynomial,
) -> Option<(Polynomial, BigRational, BigRational)> {
...
fn polynomial_exprs_equal(ctx: &Context, left: ExprId, right: ExprId, var: &str) -> bool {
    let Ok(left_poly) = Polynomial::from_expr(ctx, left, var) else {
        return false;
    };
    let Ok(right_poly) = Polynomial::from_expr(ctx, right, var) else {
...
fn split_rational_denominator_scale(ctx: &mut Context, expr: ExprId) -> (BigRational, ExprId) {
    match ctx.get(expr).clone() {
        Expr::Neg(inner) => {
...
fn polynomial_log_reciprocal_derivative_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn positive_quadratic_log_argument(
    ctx: &mut Context,
    denominator: &Polynomial,
    original_den: ExprId,
) -> ExprId {
...
fn positive_quadratic_linear_numerator_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn positive_quadratic_cube_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn positive_quadratic_over_positive_quadratic_rational_term(
    ctx: &mut Context,
    numerator: &Polynomial,
    denominator: &Polynomial,
) -> ExprId {
...
fn positive_quadratic_power_rational_term(
    ctx: &mut Context,
    numerator: &Polynomial,
    denominator: &Polynomial,
    power: i64,
...
fn polynomial_has_only_negative_terms(poly: &Polynomial) -> bool {
...
fn normalized_partial_fraction_linear_factor(factor: &Polynomial) -> Option<Polynomial> {
    if factor.degree() != 1 {
        return None;
    }
...
fn partial_fraction_log_factor_expr(ctx: &mut Context, factor: &Polynomial) -> ExprId {
    let content = positive_rational_polynomial_content(factor);
    let mut normalized = if content.is_positive() && !content.is_one() {
        factor.div_scalar(&content)
    } else {
...
fn compact_opposite_simple_log_partial_fraction_pair(
    ctx: &mut Context,
...
fn compact_opposite_simple_log_partial_fraction_antiderivative(
    ctx: &mut Context,
    terms: &LinearPartialFractionTerms,
) -> Option<ExprId> {
    if terms.len() != 2 {
...
fn linear_positive_quadratic_factors(
    denominator: &Polynomial,
...
fn multi_linear_positive_quadratic_factors(
    denominator: &Polynomial,
...
fn rational_linear_positive_quadratic_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn rational_multi_linear_positive_quadratic_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn rational_linear_positive_quadratic_required_nonzero(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Vec<ExprId> {
...
fn arcsin_polynomial_substitution_from_parts(
    ctx: &mut Context,
    numerator: &Polynomial,
    arg_poly: Polynomial,
    offset_square: BigRational,
...
fn affine_sqrt_product_derivative_solution(
    radicand: &Polynomial,
    numerator: &Polynomial,
) -> Option<Polynomial> {
...
fn affine_sqrt_product_derivative_from_parts(
    ctx: &mut Context,
    numerator: Polynomial,
    radicand: ExprId,
    var: &str,
...
fn affine_sqrt_product_derivative_div_parts(
    ctx: &Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn affine_sqrt_product_derivative_product_parts(
    ctx: &Context,
    expr: ExprId,
    var: &str,
) -> Option<(Polynomial, ExprId)> {
...
fn affine_sqrt_product_derivative_div_antiderivative(
    ctx: &mut Context,
    num: ExprId,
    den: ExprId,
    var: &str,
...
fn affine_sqrt_product_derivative_product_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn acosh_polynomial_substitution_product_antiderivative(
    ctx: &mut Context,
    expr: ExprId,
    var: &str,
) -> Option<ExprId> {
...
fn multiply_constant_integral_result(ctx: &mut Context, scale: ExprId, integral: ExprId) -> ExprId {
...
fn multiply_rational_fraction_integral_result(
    ctx: &mut Context,
    scale: ExprId,
    num: ExprId,
    den: ExprId,
...
fn multiply_rational_factor_if_possible(
    ctx: &mut Context,
    scale: ExprId,
    expr: ExprId,
) -> Option<ExprId> {
...
fn cancel_matching_factor_from_product(
    ctx: &mut Context,
    numerator: ExprId,
    denominator: ExprId,
) -> Option<ExprId> {
...
fn integral_factors_match(ctx: &Context, left: ExprId, right: ExprId) -> bool {
    if compare_expr(ctx, left, right) == Ordering::Equal {
        return true;
    }
...
fn build_product_from_factors(ctx: &mut Context, factors: &[ExprId]) -> ExprId {
    match factors {
...
mod tests {
    ...
    fn exposes_positive_quadratic_square_reduction_for_didactic_trace() {
        ...
        assert!(
            result.contains("1 / (2 * (x^2 + 1))")
                && result.contains("(1 - x^2) / (2 * (x^2 + 1)^2)"),
            "expected arctan integrand plus rational-derivative integrand, got {result}"
        );
    ...
    fn integrates_sqrt_chain_tangent_cotangent_log_quotients() {
        ...
        let expr =
            parse("k*sinh(sqrt(x)-b)/(2*sqrt(x)*cosh(sqrt(x)-b)^2)", &mut ctx).expect("parse");
        let out = integrate_symbolic_expr(&mut ctx, expr, "x").expect("integrate");
        ...
        let expr =
            parse("k*cosh(sqrt(x)-b)/(2*sqrt(x)*sinh(sqrt(x)-b)^2)", &mut ctx).expect("parse");
        let out = integrate_symbolic_expr(&mut ctx, expr, "x").expect("integrate");
