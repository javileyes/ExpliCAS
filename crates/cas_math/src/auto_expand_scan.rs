...
use crate::expr_extract::extract_scaled_log_base_argument_relaxed_view;
use crate::pattern_marks::PatternMarks;
use cas_ast::{Context, Expr, ExprId};
use num_traits::{Signed, ToPrimitive, Zero};

/// Budget limits for auto-expansion to prevent expression blow-up.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ExpandBudget {
    /// Maximum exponent to auto-expand (e.g. 6 allows (x+1)^6)
    pub max_pow_exp: u32,
    /// Maximum additive terms in the power base
    pub max_base_terms: u32,
    /// Maximum estimated generated terms after expansion
    pub max_generated_terms: u32,
    /// Maximum number of distinct variables in the base
    pub max_vars: u32,
}
...
pub(crate) fn looks_polynomial_like(ctx: &Context, id: ExprId) -> bool {
    match ctx.get(id) {
...
// NOTE: Local flatten_add REMOVED - use crate::expr_nary::add_terms_no_sign instead
...
fn try_mark_add_neg_cancellation(
    ...
    let terms = crate::expr_nary::add_terms_no_sign(ctx, id);
    if terms.len() < 2 {
        return false;
    }
