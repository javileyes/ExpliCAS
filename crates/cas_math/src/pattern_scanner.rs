use crate::expr_predicates::{is_half_expr, is_one_expr, is_two_expr};
...
fn try_match_tan_diff_identity(
    ...
    if let Expr::Function(name_a, args_a) = ctx.get(tan_a_num) {
        ...
        if cas_ast::ordering::compare_expr(ctx, args_a[0], a) != std::cmp::Ordering::Equal {
            return false;
        }
    } else {
        return false;
    ...
    if let Expr::Function(name_b, args_b) = ctx.get(tan_b_num) {
        ...
        if cas_ast::ordering::compare_expr(ctx, args_b[0], b) != std::cmp::Ordering::Equal {
            return false;
        }
    } else {
        return false;
...
fn matches_one_plus_tan_product(ctx: &Context, expr: ExprId, a: ExprId, b: ExprId) -> bool {
    ...
    let match1 = cas_ast::ordering::compare_expr(ctx, arg1, a) == std::cmp::Ordering::Equal
        && cas_ast::ordering::compare_expr(ctx, arg2, b) == std::cmp::Ordering::Equal;
    let match2 = cas_ast::ordering::compare_expr(ctx, arg1, b) == std::cmp::Ordering::Equal
        && cas_ast::ordering::compare_expr(ctx, arg2, a) == std::cmp::Ordering::Equal;
    ...
    for &factor in &factors {
        ...
        if let Expr::Function(fn_name, fn_args) = ctx.get(factor) {
            if ctx.is_builtin(*fn_name, BuiltinFn::Sin)
                && fn_args.len() == 1
                && cas_ast::ordering::compare_expr(ctx, fn_args[0], t) == std::cmp::Ordering::Equal
            {
                has_sin_t = true;
                continue;
            }
            if ctx.is_builtin(*fn_name, BuiltinFn::Cos) && fn_args.len() == 1 {
                let arg = fn_args[0];
                if cas_ast::ordering::compare_expr(ctx, arg, t) == std::cmp::Ordering::Equal {
                    has_cos_t = true;
                    continue;
                }
                ...
                if let Expr::Mul(cl, cr) = ctx.get(arg) {
                    ...
                    let is_2t = if is_two_expr(ctx, cl) {
                        cas_ast::ordering::compare_expr(ctx, cr, t) == std::cmp::Ordering::Equal
                    } else if is_two_expr(ctx, cr) {
                        cas_ast::ordering::compare_expr(ctx, cl, t) == std::cmp::Ordering::Equal
                    } else {
                        false
                    };
                    if is_2t {
        ...
        if is_two_expr(ctx, *exp) {
            if let Expr::Function(fn_id, args) = ctx.get(*base) {
                ...
                if name == "sin" && args.len() == 1 {
                    return cas_ast::ordering::compare_expr(ctx, args[0], t)
        ...
        if is_two_expr(ctx, *exp) {
            if let Expr::Function(fn_id, args) = ctx.get(*base) {
                return ctx.is_builtin(*fn_id, BuiltinFn::Tan)
                    && args.len() == 1
                    && cas_ast::ordering::compare_expr(ctx, args[0], t)
                        == std::cmp::Ordering::Equal;
            }
        }
    }
