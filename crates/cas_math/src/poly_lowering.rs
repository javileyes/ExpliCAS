...
fn try_lower_binary_poly(
    ...
    if collect_steps {
        let kind = match combined.kind {
            PolyCombineKind::Direct => PolyLowerStepKind::Direct { op },
            PolyCombineKind::Promoted => PolyLowerStepKind::Promoted,
        };
        steps.push(PolyLowerStep {
            before: expr,
            after: combined.expr,
            kind,
        });
