...
pub fn try_get_poly_result_term_count(ctx: &Context, expr: ExprId) -> Option<usize> {
    let id = crate::poly_result::parse_poly_result_id(ctx, expr)?;
    thread_local_meta(id).map(|m| m.n_terms)
}
