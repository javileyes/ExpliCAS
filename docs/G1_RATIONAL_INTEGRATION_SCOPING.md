...
### ☑ R2 — `1/(x^4-5)` (cuártica par con resolvente de raíces REALES irracionales) **[M] — HECHO** *(2026-07-15, hash en el ledger)*
...
### ☐ R3 — `1/(x^8+1)` (óctica doblemente-par) **[M] — plan verificado en scratchpad**
- Probe standalone `doubly_even_octic_antiderivative` (numerador constante, `x^8+Px^4+R`, gates exactos S=√R, s=√S ∈ ℚ, A=√(2S−P) irracional); dos niveles de descenso con coeficientes ℚ(A); radios `√(2s∓A)`. REQUIERE bumps del verificador: `ALGEBRAIC_ZERO_TEST_REDUCTION_STEPS` 512→4096 y `MAX_NODES` 500→1024 (medido por emulación fiel: ~1950 pasos). Bonus: sympy 1.14 devuelve literalmente `0` para `1/(x^8+1)` — un wrong-answer vivo; cerrarlo lo SUPERA, no solo lo iguala. Hijacks auditados: `1/(x^8-1)` (dueño earlier + gate R=−1), `1/(x^4+1)` (dueño simétrico), `x^3/(x^8+1)` (u-sub), `1/(x^4+1)²` (Ostrogradsky).
