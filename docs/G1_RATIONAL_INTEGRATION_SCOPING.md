...
- **☑ E-iv-b [S/M] — HECHO 2026-07-16 `b6fc54f37` — trazas de Newton + verificador EXACTO del RootSum.** *(Re-scopeado: los primitivos puros van antes que el nodo/render.)* `power_sums` (identidades de Newton — sumas de potencias de las raíces desde los coeficientes, sin extraer raíces), `trace_mod(h,R)=Σh(c)`, `rootsum_log_derivative_at(R,w,x₀)=Σc/(x₀−w(c))` = traza de `t·(x₀−w(t))⁻¹ mod R` (usa el inverso modular de E-iv-a), y `verify_rootsum_antiderivative(N,D,R,w)`: ambos lados son funciones racionales de grado acotado → igualdad exacta BigRational en 2·bound+1 puntos = **PRUEBA de identidad, no muestreo numérico**. Verifica los 5 casos oráculo (incl. S₅ y `1/(x^7-1)`) y REFUTA un w desplazado (adversarial). Sustituye al diferenciar-atrás surd (que no aplica a RootSum) como gate de emisión.
- **☑ E-iv-c [M] — HECHO 2026-07-16 (hash en el ledger) — nodo root_sum + driver + wiring.** `Function("root_sum", …)` opaco (patrón residual-integrate); display `RootSum(R(t), t → t·ln(x − w(t)))`; driver ensambla vía E-iv-a; inserta en el Err-arm (junto a R2/R3, tras general_rational); gate: D+R squarefree, arg lineal en x, **emisión gateada por el verificador exacto de E-iv-b**; si no → residual honesto. Gradúa `1/(x^3-x-1)`, `1/(x^5-x-1)`, `1/(x^7-1)`, `x/(x^4+x+1)` etc.

**CIERRE (2026-07-16): E-i ✅ → E-iv-a ✅ → E-iv-b ✅ → E-iv-c ✅ — la clausura universal EMITE.** Graduados: la clase Galois-obstruida completa (`x^3-x-1`, `x^5-x-1` S₅, `x^7-1`, `x^4+x+1`, `x^5-2`, `x^7-2`, `x^3-3x-1`, `x^6-2`, Φ₉) + los ℚ-factorables densos (`x^10-1`, `x^12-1` — RootSum correcto; la forma elemental bonita queda como upgrade de presentación Track R) + los Δ<0 que R2/R3 gateaban (`x^4+5`, `x^8+4`, `x²/(x^8+1)`). E-ii/E-iii ABSORBIDOS (el análisis R(t) mostró que el render radical era la vía estrecha). Residuales de presentación: eval numérico del nodo, LaTeX, Track R. Declines honestos: coeficientes simbólicos, D no-squarefree, R no-squarefree, sin argumento lineal.
