...
| **EquivalenceUnderIntroducedRequires (Definability)** | Valid if Definability-class conditions are introduced (e.g., `x≠0`, `cos(x)≠0`) | ✅ apply **and emit** `Requires:` | ✅ apply (may also emit `Assumes:` if extra heuristics are involved) | ✅ apply **only if** those "requires" can be justified or were already present (per your Strict definition) |
| **EquivalenceUnderIntroducedRequires (Analytic)** | Valid only if Analytic-class conditions are introduced (e.g., `x>0`, `base≠1`) | ⛔ block with hint ("Blocked: requires …") | ✅ apply **and record** the introduced condition (`Requires:` published; `Assumes:` if heuristics involved) | ✅ apply **only if** those "requires" can be justified or were already present (per your Strict definition) |
