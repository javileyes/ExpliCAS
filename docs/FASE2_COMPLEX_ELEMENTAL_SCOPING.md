...
#### ☐ B1 — `eval_complex` refute-only por EXTRACCIÓN de rootsum_numeric **[M]**
...
#### ☐ B3 — Log principal + builtin `Arg` EXACTO + condición `PrincipalBranch` **[L]**
- **Gradúa:** `ln(-1)→i·π`, `ln(i)→i·π/2`, `arg(i)→π/2`, `arg(-1)→π`, `arg(1+i)→π/4`, `arg(0)→undefined` explícito; cada fold emite `RequiredConditionWire` kind `PrincipalBranch` que SOBREVIVE al filtro de trivialidad. *(El graduate `ln(-e)→1+i·π` requiere el split factor-negativo-real `ln(-c)=ln(c)+i·π` con `c` probadamente positivo — extract_gaussian NO capta `-e`; añadirlo o retirar ese graduate.)*
...
#### ☐ B4 — `z^w = e^(w·Log z)` + fast-path `GaussianSqrtRule` **[M]**
- **Gradúa:** `i^i→e^(-π/2)` (cadena verificada por tramos), `2^i→cos(ln 2)+i·sin(ln 2)` (**el test de aceptación de `2^i` vive AQUÍ**, requiere B2), `sqrt(i)→√2/2·(1+i)` EXACTO vía fast-path, `sqrt(3+4i)→2+i`, `(1+i)^(1/2)` polar honesto.
