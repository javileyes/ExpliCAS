...
- **FIXED (commit 7da3d3f09): abs-equation-with-linear-term family** (P1) — `solve(|x²-2x|=3) → {-1,3}` via the `|arg|=c → arg=±c` split (was a circular residual). Variable-RHS abs cases deferred.

- **FIXED (commit ddf753b7a): complex principal-branch odd-root** (P0) — `(-1)^(1/3)` in complex mode is now `1/2 + (√3/2)i` (principal branch), not the real odd root `-1`. **All 8 Round-5 P0 defects are now fixed.**
