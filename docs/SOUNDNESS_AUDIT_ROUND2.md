...
`0^sin(0)`, `1/sinh(x−x)`-style denominators all leaked. **Fix:** a new exact
`special_function_value(ctx, expr) -> Option<BigRational>` (used by `is_provably_zero`
as a direct arm for bare `f(arg)`, plus an additive-sum pass for `cos(0)−1`-style
nodes). The frozen, EXACT table: vanish-at-provably-zero-argument (`sin`, `tan`,
`sinh`, `tanh`, `asin`/`arcsin`, `atan`/`arctan`, `asinh`, `atanh` → 0, so `sin(x−x)`
is covered too), equal-1-at-provably-zero (`cos`, `cosh`, `sec`, `exp` → 1, plus the
`e^0 = Pow(E, ·)` spelling), vanish-at-literal-one (`ln`, `log`, `log2`, `log10`,
`acos`/`arccos`, `acosh` → 0, and two-arg `log(b, 1) = 0`). Every entry is an exact
rational at a point where the function is defined, so it NEVER reports a false zero:
a symbolic argument (`sin(x)`), an irrational value (`sin(1)`, `ln(2)`, `cos(π/4)`,
`arctan(1)`), a wrong special point (`cos(0)=1≠0`), `e^2`/`e^x`, and the poles
(`cot(0)`, `csc(0)`, `ln(0)`) all decline. Being a `is_provably_zero` improvement it
benefits EVERY zero-denominator family (R3-3/R4-2/R4-3/R4-4/R4-6/R3-2) at once.
guardrail+pressure fingerprints BYTE-IDENTICAL. **Deferred:** trig special angles
(`sin(π/6)=1/2`, `tan(π/4)=1` — need rational-π recognition + surd values), a general
`is_provably_one` (this cycle: literal `1` only), and `ln(0)`/π-2-poles (above).
