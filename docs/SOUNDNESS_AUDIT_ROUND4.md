...
- **(H1) f64 log-ratio fold → false `AllReals`.** The simplifier folds `log(a)/log(b) − <decimal>` to a literal `0` on f64-closeness, so `eval "log(2)/log(3) - 0.6309297535714574"` already returns `0` though the true residual is `+3.7e-17` (a transcendental ratio cannot equal a finite decimal). The exact EqZero prover then correctly reports `AllReals` for the literal `0` it is handed — the unsoundness is the upstream `evaluator_f64` log-fold gate (the classic "soundness gates must be exact" failure), not the prover. Scope: log-ratio-specific (`pi-3.14159…`, `exp(1)-2.71828…` correctly stay `Empty`).
- **(H2) trig-product → false `Empty`.** A constant that is exactly zero by a `sin`/`tan` product identity the simplifier cannot reduce to its `sqrt(n)/2^k` closed form (`sin(pi/7)sin(2pi/7)sin(3pi/7)−sqrt(7)/8`, `tan(pi/5)tan(2pi/5)−sqrt(5)`, `sin(pi/9)sin(2pi/9)sin(4pi/9)−sqrt(3)/8`) is committed to a definite `No solution` instead of degrading to the honest conditional its structural siblings (`tan(pi/7)tan(2pi/7)tan(3pi/7)−sqrt(7)`, `cos(pi/7)cos(2pi/7)cos(3pi/7)−1/8`) correctly use. The classifier asserts "provably nonzero" for a constant it has not exactly proven nonzero.

These two are the NEXT soundness candidates (H2 lives in the var-eliminated classifier's nonzero path — punt to the honest conditional when nonzero-ness is not exactly proven; H1 is an `evaluator_f64` gate that must not fold a near-decimal to `0`). The remaining NON-soundness items are COMPLETENESS (deciding ln/exp/trig VALUES).
