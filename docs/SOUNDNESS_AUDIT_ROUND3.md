...
fixed (`3837ad2db4a11747396f629832caf1f409778dad`: the var-free residual `−ln(c)` is recognised as a provably-non-zero constant ⇒ `Empty`). **Remaining:**
...
**Fix — `ln(x)=ln(-x)→ℝ` collapse (commit `8aaafaa7e76d3ac134a2402cf3537eea41bbd121`).** `solve(ln(x)=ln(-x),x)` returned "All real numbers": it collapses to an identity but the engine *does* record the required conditions `Positive(x)` (from `ln(x)`) and `Positive(-x)` (from `ln(-x)`) — i.e. `x>0 ∧ x<0`, a contradictory domain whose real solution set is EMPTY. An `AllReals` result means "every real satisfying the required conditions", so when those conditions are mutually unsatisfiable it must collapse to `Empty`. The final filter `filter_real_solutions` (which already receives `ctx.required_conditions()`) now adds an `AllReals` arm that returns `Empty` when `required_conditions_are_contradictory`: a `Positive(a) ∧ Positive(b)` pair whose targets are negations (`a == -b`, via `cas_math::poly_compare::poly_relation` → `Negated`). Result: `solve(ln(x)=ln(-x),x)`, `solve(ln(-x)=ln(x),x)`, `solve(log(x)=log(-x),x)`, `solve(ln(x-1)=ln(1-x),x)`, `solve(ln(x)+ln(-x)=0,x)` → "No solution". Strictly conservative — the check is on STRICT `Positive` only: a genuine identity on a non-empty domain stays `AllReals` (`ln(x)=ln(x)`, `x+1=x+1`, `sin²+cos²=1`), and a `NonNegative(a) ∧ NonNegative(-a)` pair meets at `0` (`sqrt(x)=sqrt(-x) → {0}`) so it is NOT collapsed. Guardrail+pressure byte-identical; full `cargo test --workspace` green. (Note: the user-facing eval/CLI path resolves the var-eliminated residual to `AllReals` before this filter and is now correct; the bare `cas_solver::solve` API returns the unresolved `Residual` for these — honest, not a false `AllReals` — so the contradiction collapse is covered by a direct unit test on `required_conditions_are_contradictory`.) The 143-probe adversarial sweep confirmed the fix is clean (zero false-collapses; identities and the `NonNegative` boundary intact) but **surfaced a distinct PRE-EXISTING residual — now FIXED (commit `3837ad2db4a11747396f629832caf1f409778dad`):** `solve(ln(x)=ln(c·x),x)` for `c≠1` (e.g. `ln(x)=ln(2x)`, `ln(x)=ln(3x)`, `ln(x+1)=ln(2x+2)`) returned "All real numbers", but the real solution set is EMPTY — the only algebraic root `x=0` is excluded by the `x>0` domain. This is a *single*-condition case (`x>0`), so the `Positive(a)∧Positive(-a)` contradiction check does not apply; instead `ln(x)−ln(c·x)` cancels its variable to leave the var-free *symbolic* constant `−ln(c)`, which the engine wrongly classified as a `Constraint` (→ `AllReals`) rather than a contradiction. Root cause: `classify_var_free_difference` (`crates/cas_solver_core/src/solve_outcome.rs`) only treated a bare `Number(nonzero)` literal as `ContradictionNonZero`; a var-free symbolic constant that is provably non-zero fell through to `Constraint`. Fix: a sound, conservative `is_provably_nonzero_constant` helper now also returns a contradiction for (a) any folded rational `≠0` (via `numeric_eval::as_rational_const`, EXACT — never a float gate), (b) signed/scaled single-arg logs `±k·ln(c)` / `log2`/`log10` of a rational `c>0, c≠1`, and (c) two-arg `log(b,c)` with `b,c` rational `>0, ≠1` — all exact because `log_b(c)=0 ⟺ c=1`. Anything not provably non-zero stays `Constraint` (no false contradictions). It lives in the *core* solve outcome, so both the eval/CLI path and the bare `cas_solver::solve` API are corrected. Now: `solve(ln(x)=ln(2x),x)`, `solve(ln(x)=ln(x)+ln(2),x)`, `solve(log(2,x)=log(2,3x),x)` → "No solution"; the `c=1` boundary (`ln(x)=ln(1·x)`, `ln(x)+ln(1)`) and genuine identities (`ln(x)=ln(x)`, `x+1=x+1`) stay "All real numbers"; genuine solutions (`ln(x)=ln(2)→{2}`, `log(2,x)=3→{8}`) untouched. Guardrail+pressure byte-identical; full `cargo test --workspace` green; 5 direct unit tests on the classifier (negated/scaled log, two-arg log, the `c=1` boundary). **Follow-up extension (commit `6d976c233931763d76cfceee885e724194ad5ca1`):** the 104-probe adversarial sweep of this fix confirmed it clean (zero false contradictions, zero regressions) and surfaced a same-family over-claim — `solve(ln(2x)=ln(x)+1,x)` → "All real numbers", residual `ln(2)−1` (a natural log mixed with a rational), which the original recognizer (`±k·ln(c)` and folded rationals only) did not cover. Extended `is_provably_nonzero_constant` with a `rational ± k·ln(c)` arm via a new `is_irrational_natural_log` predicate: `ln(c)` is irrational for every rational `c>0, c≠1` (if `ln(c)=p/q` then `c=e^{p/q}` is transcendental, contradicting `c` rational; `p/q=0 ⇒ c=1`), so `rational + irrational = irrational ≠ 0`. **Soundness boundary (load-bearing): this uses the NATURAL log only** — `log2`/`log10`/two-arg `log(b,c)` are excluded because they can be rational (`log2(8)=3`), so `log(2,8x)=log(2,x)+3` (residual `log(2,8)−3 = 0`, a genuine identity) correctly stays "All real numbers" rather than collapsing to a false contradiction (regression-guarded by a unit test). Now `solve(ln(2x)=ln(x)+1,x)`, `solve(ln(3x)=ln(x)+2,x)` → "No solution"; guardrail+pressure byte-identical, 3 more unit tests. Honest residuals still open (over-report "All real numbers", never a false claim): non-rational/transcendental constants the prover does not cover — `±π`, `±e`, `±√2`, a product of two logs `ln(2)·ln(3)` — and the `ln(c) + rational` case where `c` is irrational.

**Panic fix — mixing `ln` and two-arg `log(b,c)` crashed the simplifier (commit `71e168b4bf94b2241f2b5d5aa21fabd32a2648cf`).** The focused adversarial sweep of the rational±log extension (61 probes) confirmed it clean — boundary preserved (`log(2,8x)=log(2,x)+3` stays "All real numbers"), zero false contradictions — and **bisection against the pre-fix tree proved every newly-flagged issue PRE-EXISTING (my fix has zero regressions; the adjudicator's `is_regression_from_fix` was a misattribution).** One was severe: `solve(ln(2x)=ln(x)+log(2,8),x)` (and any sum mixing a natural log with a two-arg `log(b,c)`, e.g. `ln(2x)-ln(x)-log(2,8)`) **panicked the process** (exit 101, `index out of bounds` in `Context::get`). Root cause: `try_extract_log_parts` returns `ln_base_sentinel()` (`ExprId::from_raw(u32::MAX)`, a non-arena id) as the base of a natural log, and `log_terms_match_up_to_abs_subject_for_cancellation` (`crates/cas_engine/src/rules/arithmetic.rs`) compared that base against a real base (the `2` of `log(2,8)`) with the sentinel-unaware `exprs_match_for_cancellation` → `compare_expr` → out-of-bounds deref. Fix: compare log bases with the sentinel-aware `cas_math::logarithm_inverse_support::bases_equal_for_logs` (already used for this purpose, resolves the sentinel to `e` without dereferencing it). Behavior-preserving for all non-sentinel bases (only the previously-crashing path changes); guardrail+pressure byte-identical except the one new regression test's count. With the panic gone, the residual `ln(2)−3` is then correctly classified non-zero, so `solve(ln(2x)=ln(x)+log(2,8),x)` → "No solution". **Family A fix — non-natural-log over-claim (commit `19835056d9b8706cedf763c56ddba7ca9c8b969e`).** The pre-existing sibling: `solve(log_b(c·x)=log_b(x)+k,x)` for NON-natural logs where the residual `k·log_b(c)−k′` is a nonzero non-natural-log constant (e.g. `log(2,3x)=log(2,x)+1`, residual `log₂3−1`) returned "All real numbers" but the real set is empty. Closed with the EXACT `log_b(c)=m ⟺ c=b^m` rational-power recognizer (not irrationality — `log₂(8)=3` is rational): `is_provably_nonzero_constant`'s `rational ± log` arm now also extracts a rational-base log `±k·log_b(c)` (`as_rational_base_log_term`, covering `log2`/`log10`/two-arg `log(b,c)` with `b,c` rational `>0, ≠1`) and tests `rational_power_provably_not_equal(b, m, c)` — `b^(p/q)=c ⟺ b^p=c^q`, computed exactly with bounded integer exponents; an undecidable (over-large) exponent returns *false* (stays a vacuous `Constraint`, never a false contradiction). **Soundness boundary verified with FRACTIONAL exponents:** `log(4,8x)=log(4,x)+3/2` (residual `log₄8−3/2`, and `8=4^(3/2)`) and `log(9,27x)=log(9,x)+3/2` stay "All real numbers" — the exact test confirms `4^3=8^2=64`, so the identity is NOT collapsed. Now `solve(log(2,3x)=log(2,x)+1,x)`, `solve(log10(30x)=log10(x)+1,x)`, `solve(log(3,5x)=log(3,x)+1,x)` → "No solution"; genuine solutions (`log(2,x)=3→{8}`) and identities untouched. Guardrail+pressure byte-identical; 4 new unit tests (incl. a direct exact-power test over integer/fractional/negative exponents). With Families A+B closed, the entire `log_b(c·x)=log_b(x)+k` / `ln`-mix family is sound (no over-claims, no crashes); the honest residuals that remain are only truly-transcendental constants (`±π`, `±e`, `±√2`, products of logs). **Family A's own adversarial sweep (105 probes) re-confirmed the exact recognizer is sound — 168 identities incl. fractional/large exponents all stayed all-reals, the feared false contradiction never occurred — and surfaced one more PRE-EXISTING sibling, now FIXED (commit `56a56122c50d6e3dcfd0326c252621d89799205d`):** a NEGATIVE coefficient inside the log argument, e.g. `solve(log(2,-8x)=log(2,x)+3,x)`, returned "All real numbers" though its recorded domain `Positive(-8x) ∧ Positive(x)` (i.e. `x<0 ∧ x>0`) is empty — internally inconsistent with `solve(log(2,-x)=log(2,x)+1,x)` → "No solution" under the *identical* conditions. The `required_conditions_are_contradictory` check (from the `ln(x)=ln(-x)` collapse) only detected exact negation `a == -b`; generalized to negative proportionality `a = λ·b, λ < 0` via the new exact `cas_math::poly_compare::poly_negatively_proportional` (leading-coefficient ratio + `a − λ·b == 0` verification over `MultiPoly`), so `Positive(-8x) ∧ Positive(x)` now collapses to `Empty`. Strictly conservative: still STRICT `Positive` only (NonNegative pairs like `sqrt(x)=sqrt(-x) → {0}` meet at 0 and are untouched), and a POSITIVE multiple (`Positive(2x) ∧ Positive(x)`) is satisfiable and not flagged. Now `solve(log(2,-8x)=log(2,x)+3,x)`, `solve(ln(-2x)=ln(x),x)` → "No solution"; guardrail+pressure byte-identical, full `cargo test --workspace` green (3 new poly_compare tests + extended contradiction test). (Separately, verification surfaced a PRE-EXISTING bug unrelated to this filter — now **FIXED (commit `1a0806509ff3ee3c2226af5cfa6f19e88e0b5138`)**: `solve((x^2-2x-1)/(x-1+sqrt(2))=0,x)` returned "No solution" and `solve((x^2-2x-1)/(x-1)=0,x)` returned a malformed `solve(x = sqrt(2x+1))` residual. Root cause: the Div-isolation route re-solved the numerator via the isolation-only reentry, which √-isolates a quadratic-with-linear-term (`x^2-2x-1=0 → x=√(2x+1)`, recursive depth-overflow) instead of using the Quadratic strategy — even though the same numerator solves correctly standalone. Fix: in `crates/cas_solver_core/src/solve_runtime_flow_isolation_custom_pow.rs`, the Div route's numerator re-solve now routes a degree-2 *equality* equation — an expanded `x^2-…` polynomial OR a factored product of ≥2 var-bearing factors `(x-a)(x-b)` — through the full strategy-selecting solve (the callback the Mul route already uses), so the Quadratic strategy claims it; the denominator-zero root is excluded by the usual NonZero machinery (`1-√2` for `den=x-1+√2` ⇒ `{1+√2}`). Gated to `op == Eq` so the inequality interval routes are untouched. Now: `(x^2-2x-1)/(x-1)=0 → {1-√2, 1+√2}`, `(x^2-2x-1)/(x-1+√2)=0 → {1+√2}`, `(x^2-4x+1)/(x-3)=0 → {2±√3}`. Verified: factorable/constant-numerator/reciprocal/linear cases unchanged, michaelis-menten step text preserved, full `cargo test --workspace` green, guardrail+pressure byte-identical.)
