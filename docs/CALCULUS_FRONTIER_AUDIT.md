...
| Límites | ~70-80% *(re-sondeado 2026-07-15)* | ~65-75% narrado | motor maduro: factor-cancela, notables (sin/x, (1−cos)/x², **`e` vía `(1+1/x)^x`** ✅), L'Hôpital ITERADO, squeeze/sándwich, jerarquía ∞/∞ (ln≪pot≪exp) — todos narrados es/en. Narra también el ∞−∞ (conjugado al ∞ y común-denom en punto finito, `1/x−1/sin x`/`1/tan²x−1/x²`). Residuales: VALOR — las formas 0·∞/0^0 RESUELVEN unilaterales (`x·ln x @0+ → 0`, `x^x @0+ → 1`, ya graduada); solo el BILÁTERO declina, CORRECTO (ninguna es real para x<0). El algoritmo de valor sigue siendo un allowlist de patrones (no Gruntz). `oo` no parsea como dirección (usar `infinity`). NARRACIÓN pendiente: desglose de las unilaterales 0·∞/0^0 (paso genérico único) y del ∞−∞ común-denom con denominador `log/exp` (`1/(x−1)−1/log x`, cuyo VALOR ya da `−1/2` pero sin traza). *(Nota: la fila previa "~45-50%, allowlist, `e` inalcanzable" era STALE — falsificada al imprimir los SUBSTEPS, no solo el `rule` de nivel-1.)* |
| Definidas/impropias | ~70-75% | ~35-40% | FTC/touches/impropias elementales sólidos; pre-simplificador sabotea casos |
| Calidad educativa | ~58% bien narrado | ~35-40% | condiciones de dominio sistemáticas (punto fuerte); por partes narrada en las TRES asignaciones u/dv: polinomio·ln, polinomio(lineal)·{eˣ,sin,cos,sinh}, y u=función/dv=dx (inverse-trig/hiperbólicas + ln(x) solo). La **repetida grado≥2 YA se narra** como "por partes repetida" (`x²·sin x`, `x²·eˣ` con varios ciclos u/dv + "Integrar el término restante", 2026-07-15); queda solo el **cíclico eˣ·sin** (valor correcto pero narrado como expandir+factor, no como ciclo por partes) |
...
- [x] **(S) Ecuaciones polinómicas en x^(1/q) fugaban residual malformado y perdían todas las raíces**:
  ...
  *(peldaño A3 graduado 2026-06-24 4b59f0b8c — CIERRA Cluster A: `√(x+3)+√x=3` etc. fugaban; ahora `{1}`,
...
- [~] **(F) Cuárticas+ irreducibles**: `1/(x^4+1)`, `1/(x^6-1)` —
  ...
  *(G1 Cap. A GRADUADO d557556ea 2026-07-14: el factor cuadrático racional con Δ>0 (raíces
  ...
  *(G1 Cap. B GRADUADO `6c4d59afc` 2026-07-14: la cuártica par IRREDUCIBLE como FACTOR de un
...
- [ ] **Cociente simbólico cancelado sin plegar (`(2*x)/(5*x)`→`"2 / 5"` plain vs `"2/5"` steps):** PELDAÑO
  ...
  *(2026-06-26 INTENTADO y revertido: plegar el `Div(Number,Number)` en `try_exact_common_factor_mul_fraction_preorder`
  ...
  diferido. La cancelación PARCIAL multi-factor `(2*x*y)/(5*x*y)→"2·x/(5·x)"` SÍ se cerró 2026-06-26 c1a6a727c:
