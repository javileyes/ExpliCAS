...
- [x] sign-via-abs: `−|x|/x=1` / `c·|x|/x` (constant-RHS) CERRADO batch-2 ciclo 3; RHS-VARIABLE `x/|x|=x`→{1,−1}, `x/|x|=−x`→No solution CERRADO batch-3 ciclo 2 (split en sign(g)=±1). NOTA: la "correct answer" del audit para `x/|x|=−x` (`{−1,1}`) era ERRÓNEA; el real es No solution.
