...
#### O4 — 2º orden homogénea coef. constantes **[S/M]** ☑ *(graduado 2026-07-22, hash en el ledger: O20-O25 por las 3 ramas del disc BigRational interno + V31 + IVP en las 3 ramas incl. envelope compleja; O23 emite Y verifica (gate por-base D5); surds exactos (β=√3, raíces áureas con φ); shape anidado; hallazgo doble: las ecuaciones del IVP salen de las BASES (derivar la general con C1/C2 quema el budget — misma familia hostil) y verificar y_p contra la EDO es superfluo POR TEOREMA (linealidad) además de hostil; Z2/Z6 residuales con causa migrada a variable_coefficients; familia `coef_const_2o` 29 casos)*
...
#### O5 — 2º orden no-homogénea (coeficientes indeterminados) **[M — el trabajo neto-nuevo mayor]** ☑ *(graduado 2026-07-22, hash en el ledger: N26-N29 + resonancia doble s=2 + trig resonante + productos x·e^kx + IVP no-homogéneo; el collector por función-base corre sobre derivadas del differ PURO aplanadas estructuralmente — jamás el simplificador (C5-hostil) — con worklist de factores y litter ln(e) normalizada; Gauss BigRational propio (sin solve_system); gate afín L[y_p]−RHS→0 como red final; N27 SIN el falso-rojo de stderr del scoping — la ruta estructural no pasa por el pipeline; bug cazado: el gate por-base de O4 verificaba contra la ecuación completa en vez de la homogénea asociada — toda no-homogénea declinaba; tan(x) → residual honesto nombrando variación de parámetros; familia `UC` 35 casos)*
...
#### O6 — Sistemas 2×2 `X' = AX` **[M]** ☑ *(graduado 2026-07-22, hash en el ledger: Y34 textbook + Y35 REAL vía Re/Im (sin `i` en la salida — el complejo vive y muere dentro del método) + Y-def por vector generalizado (A−λI)w=v; forma lista wire+REPL (el REPL reusa el build completo); IVP de sistemas → decline honesto pineado (decisión O6); irracionales → residual nombrado; gate por componente 2×2=4; pin D17 cumplido: los verbos eigen intactos; familia `sistema` 45 casos — con esto la capacidad núcleo O0-O6+O8 de la fase queda CERRADA)*
...
- ☑ Extensión opcional hermana (mismo estatus): **factor integrante μ(x) simple** para no-exactas (`μ = e^∫((M_y−N_x)/N)` — mapper 2 la aisló como ciclo M aparte). *(graduado 2026-07-23 commit `5f4a0727b`: μ(x) Y μ(y) por test de exactitud plegado + delegación al handler exacto con sus gates D5/D11; warning de honestidad por soluciones singulares donde μ se anule; peldaño: potenciales RACIONALES en el reconstructor exacto — el caso μ(y)=y⁻² queda residual honesto por techo pre-existente del delegado)*
- ☑ Extensión opcional hermana 2: **Cauchy-Euler** `x²y''+axy'+by=0` — S puro sobre el molde D9 (característica `r(r−1)+ar+b=0`, mismas 3 ramas); HOY `solve(x^2*diff(y,x,2)+x*diff(y,x)-y=0,y)` → `{0}` (colapso probado) y hasta su ciclo queda cubierta por el decline Z-coef-variables — el residual la nombra para no ambiguar el contrato. *(graduado 2026-07-23, hash en el ledger: indicial exacta interna con las 3 ramas — x^r distintas / x^r·ln x doble / x^α·trig(β·ln x) compleja — matcher estructural x_pow==orden-de-derivada que descarta Bessel DE FORMA, gate D5 por base contra la EDO cruda; el colapso probado del scoping MIGRÓ a resolución; quedan como peldaños IVP-CE (x0=0 singular) y raíces indiciales irracionales)*
